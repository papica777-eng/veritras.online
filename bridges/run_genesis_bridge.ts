...
import { GenesisBridgeAdapter, AxiomType, MarketOpportunity } from './modules/GenesisBridgeAdapter'; // Importing TS source
import { PineconeBridgeSystem } from './modules/PineconeBridgeSystem';
import { NeuralHUD, WaveType, ModuleSource } from './neural-hud'; // Importing from root
...
async function IgniteGenesisBridge() {
    ...
    const bridgeSystem = new PineconeBridgeSystem({
        apiKey: apiKey || 'mock-key',
        indexName: index,
        host: process.env.PINECONE_HOST
    });

    const genesisAdapter = new GenesisBridgeAdapter({
    ...
    genesisAdapter.on('axiomStored', (data) => {
        ...
        hud.emitWave({
            ...
            source: 'neural_optimizer' as any, // Using a valid-ish source or casting
            confidence: 1.0,
            duration: 100,
            relatedWaves: []
        });
