...
import { StealthLayer, HumanSimulator, WorkerPool } from './index';
...
// ─────────────────────────────────────────────────────────────────────────────
// VERITRAS B2B TARGETS — Expanded target list
// ─────────────────────────────────────────────────────────────────────────────
...
interface Target {
    ...
    email: string;
    company: string;
    industry: 'fintech' | 'saas' | 'ecommerce' | 'hosting' | 'recruitment' | 'other';
    country: 'BG' | 'EU' | 'US';
    priority: number;
}
...
async function runEmailCampaign(pool: WorkerPool): Promise<void> {
...
async function runXCampaign(pool: WorkerPool): Promise<void> {
...
async function runLinkedInCampaign(pool: WorkerPool): Promise<void> {
    pool.enqueue({
        channel: 'linkedin',
...
async function runVeritrasDay(): Promise<void> {
    console.log(`
╔══════════════════════════════════════════════════════════════════════════════╗
...
`);
    ...
    const pool = new WorkerPool(3, DRY_RUN);
    const channels = process.argv.find(a => a.startsWith('--channel='))?.split('=')[1];

    if (!channels || channels === 'email') await runEmailCampaign(pool);
    if (!channels || channels === 'x') await runXCampaign(pool);
    if (!channels || channels === 'linkedin') await runLinkedInCampaign(pool);
...
async function autonomousLoop(): Promise<void> {
    ...
    while (true) {
        ...
        await StealthLayer.humanDelay(remaining * 0.95, remaining * 1.05); // ±5% jitter on interval
    }
}
