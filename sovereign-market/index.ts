...
import { MarketBlueprint, BlueprintSource, CrawlDataInput } from '../src/departments/biology/evolution/MarketBlueprint';
...
export class StealthLayer {
    ...
    static generateFingerprint(): Record<string, string | number | boolean> {
        return {
    ...
    static getJitteredHeaders(baseHeaders: Record<string, string>): Record<string, string> {
        const jitter: Record<string, string> = { ...baseHeaders };
    ...
    static isBlockResponse(statusCode: number, body: string): boolean {
        if ([429, 403, 503, 521, 522].includes(statusCode)) return true;
    ...
    static async backoff(attempt: number, baseMs: number = 2000): Promise<void> {
...
export class HumanSimulator {
    ...
    private actionsThisSession: number;
    ...
    async simulateRead(contentLength: number): Promise<void> {
        ...
        await StealthLayer.humanDelay(
            Math.min(500, naturalRead * 0.3),
            Math.min(5000, naturalRead)
        );
    }
    ...
    async restIfNeeded(): Promise<void> {
        if (this.shouldRest()) {
...
// ─────────────────────────────────────────────────────────────────────────────
// WORKER POOL — Autonomous Thread Orchestration
// ─────────────────────────────────────────────────────────────────────────────

export interface WorkerTask {
    id: string;
    channel: 'email' | 'x' | 'linkedin' | 'webhook' | 'scan' | 'android';
...
export class WorkerPool {
    ...
    private resultsLog: WorkerResult[] = [];
    private logPath: string;
    private dryRun: boolean;

    constructor(maxConcurrent: number = 3, dryRun: boolean = false) {
    ...
    enqueue(task: Omit<WorkerTask, 'id'>): string {
        const id = crypto.randomBytes(8).toString('hex');
        const fullTask: WorkerTask = { id, ...task };
    ...
    private async runInlineTask(task: WorkerTask, start: number): Promise<void> {
        const wId = task.id;
        this.activeWorkers.set(wId, null as unknown as Worker); // Mark as active
    ...
    private async dispatchToHandler(task: WorkerTask): Promise<{ success: boolean; data?: Record<string, unknown>; error?: string }> {
    ...
    private async handleEmailTask(task: WorkerTask): Promise<{ success: boolean; data?: Record<string, unknown>; error?: string }> {
        ...
        const blueprintEngine = new MarketBlueprint({
    ...
    private async handleXTask(task: WorkerTask): Promise<{ success: boolean; data?: Record<string, unknown>; error?: string }> {
    ...
    private async handleLinkedInTask(task: WorkerTask): Promise<{ success: boolean; data?: Record<string, unknown>; error?: string }> {
    ...
    private async handleAndroidTask(task: WorkerTask): Promise<{ success: boolean; data?: Record<string, unknown>; error?: string }> {
    ...
    private async handleWebhookTask(task: WorkerTask): Promise<{ success: boolean; data?: Record<string, unknown>; error?: string }> {
    ...
    private async handleScanTask(task: WorkerTask): Promise<{ success: boolean; data?: Record<string, unknown>; error?: string }> {
    ...
    getStats(): { total: number; success: number; failed: number; byChannel: Record<string, number> } {
...
// ─────────────────────────────────────────────────────────────────────────────
// CAMPAIGN ROUTER — Orchestrates campaigns across channels
// ─────────────────────────────────────────────────────────────────────────────

export interface Campaign {
    name: string;
    channels: Array<'email' | 'x' | 'linkedin' | 'webhook' | 'android'>;
    targets: Array<{ domain?: string; email?: string; name?: string; appId?: string; action?: string }>;
...
`);
    ...
    const activeCampaigns = channelFilter
        ? DEFAULT_CAMPAIGNS.filter(c => c.channels.includes(channelFilter as Campaign['channels'][0]))
        : DEFAULT_CAMPAIGNS;
