...
export class MindEngineAPI extends APIServer {
        ...
        case 'vortex': {
          ...
          const scan = (dir: string) => {
            try {
              ...
              return new Promise((resolve, reject) => {
                addLog(`[System] Spawning QAntum CLI: ${module} ${args.join(' ')}`);

                const cliPath = path.join(process.cwd(), '../qantum-cli.js'); // Adjust path to root
  ...
  private setupDashboardRoutes() {
    ...
    this.get('/api/dashboard', async (req, res) => {
      // In a real scenario, we would parse `qantum status` output
