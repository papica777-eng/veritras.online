...
function addResult(category: string, test: string, status: AuditResult['status'], message: string, details?: string): void {
...
function getAllFiles(dir: string, extensions: string[] = ['.ts', '.js']): string[] {
    const files: string[] = [];
...
function auditModuleStructure(): void {
    // Complexity: O(1)
    log('📦', '\n═══ 2. Module Structure ═══\n');
...
function auditFunctionSignatures(): void {
    ...
    // Complexity: O(1)
    addResult('Functions', 'Return Types', percentage >= 80 ? 'PASS' : 'WARN', 
        `${percentage}% of functions have return types (${typed}/${typed + untyped})`);
    
    if (untypedList.length > 0) {
        // Complexity: O(1)
        addResult('Functions', 'Any Types', 'WARN', 
            `Files with many 'any' types`, untypedList.join(', '));
    } else {
        // Complexity: O(1)
...
function auditImports(): void {
    // Complexity: O(N*M) — nested iteration detected
    log('📥', '\n═══ 4. Imports & Exports ═══\n');
    
    const files = getAllFiles('./src', ['.ts']);
    let circularRisk: string[] = [];
    ...
    if (circularRisk.length > 0) {
        // Complexity: O(1)
        addResult('Imports', 'Circular Imports', 'WARN', 
            `${circularRisk.length} potential circular imports`, circularRisk.join(', '));
    } else {
        // Complexity: O(1)
    ...
    // Complexity: O(1)
    addResult('Exports', 'Index Files', hasIndex >= srcSubdirs.length / 2 ? 'PASS' : 'WARN',
        `${hasIndex}/${srcSubdirs.length} modules have index.ts`);
}
...
function auditSecurity(): void {
    ...
    try {
        const gitLog = execSync('git log --oneline -20 2>&1', { encoding: 'utf-8' });
...
function auditDependencies(): void {
    ...
    try {
        ...
        try {
            const outdated = execSync('npm outdated --json 2>&1', { encoding: 'utf-8' });
        ...
        try {
            const audit = execSync('npm audit --json 2>&1', { encoding: 'utf-8' });
            ...
            if (vulns) {
                ...
                if (total > 0) {
                    // Complexity: O(1)
                    addResult('Dependencies', 'Vulnerabilities', 'WARN', 
                        `${total} vulnerabilities (${vulns.critical} critical, ${vulns.high} high)`);
                } else {
                    // Complexity: O(1)
...
function auditTests(): void {
    ...
    // Complexity: O(1)
    addResult('Tests', 'Test Files', totalTests > 0 ? 'PASS' : 'WARN', 
        `${totalTests} test files found`);
    ...
    // Complexity: O(1)
    addResult('Tests', 'Test Cases', testCases > 10 ? 'PASS' : 'WARN', 
        `${testCases} test cases`);
    
    // Check if tests pass
    try {
        // Complexity: O(1)
        execSync('npm test 2>&1', { encoding: 'utf-8', timeout: 60000 });
        // Complexity: O(1)
...
function auditCodeQuality(): void {
    ...
    if (largeFiles.length > 0) {
        // Complexity: O(1)
        addResult('Quality', 'File Size', 'WARN', 
            `${largeFiles.length} files over 500 lines`, largeFiles.slice(0, 5).join(', '));
    } else {
        // Complexity: O(1)
    ...
    if (longFunctions.length > 0) {
        // Complexity: O(1)
        addResult('Quality', 'Function Size', 'WARN', 
            `${longFunctions.length} functions over 100 lines`, longFunctions.slice(0, 5).join(', '));
    } else {
        // Complexity: O(1)
    ...
    // Complexity: O(1)
    addResult('Quality', 'TODOs', todoCount > 10 ? 'WARN' : 'PASS', 
...
function auditDocumentation(): void {
    // Complexity: O(N) — linear iteration
    log('📚', '\n═══ 9. Documentation ═══\n');
    ...
    for (const doc of requiredDocs) {
        if (fs.existsSync(doc)) {
            ...
            // Complexity: O(1)
            addResult('Docs', doc, content.length > 100 ? 'PASS' : 'WARN', 
                content.length > 100 ? 'Present and detailed' : 'Present but minimal');
        } else {
            // Complexity: O(1)
            addResult('Docs', doc, doc === 'README.md' || doc === 'LICENSE' ? 'FAIL' : 'WARN', 
                'Missing');
        }
    }
    ...
    // Complexity: O(1)
    addResult('Docs', 'API Documentation', apiDocs.length >= 3 ? 'PASS' : 'WARN', 
        `${apiDocs.length} documentation files in docs/`);
    ...
    // Complexity: O(1)
    addResult('Docs', 'JSDoc Coverage', coverage >= 50 ? 'PASS' : 'WARN', 
        `${coverage}% functions documented (${jsdocCount}/${functionCount})`);
}
...
function auditBuildScripts(): void {
    // Complexity: O(N) — linear iteration
    log('🔨', '\n═══ 10. Build & Scripts ═══\n');
...
function generateReport(): FinalReport {
...
function saveReport(report: FinalReport): void {
