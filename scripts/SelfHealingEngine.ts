...
export class SelfHealingEngine extends EventEmitter {
  ...
  /**
   * Extract context from existing element or DOM (Shadow DOM aware)
   */
  private async extractContext(selector: string): Promise<ElementContext> {
    ...
    try {
      const context = await this.page.evaluate((sel) => {
        // Deep query selector function to penetrate shadow roots
        const deepQuerySelector = (selector: string, root: Document | Element | ShadowRoot = document): Element | null => {
          ...
          for (const item of Array.from(all)) {
            if (item.shadowRoot) {
              const found = deepQuerySelector(selector, item.shadowRoot);
