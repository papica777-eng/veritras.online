/**
 * visual-phishing-detector — Qantum Module
 * @module visual-phishing-detector
 * @path scripts/CyberCody/src/modules/visual-phishing-detector.ts
...
const KNOWN_BRANDS: BrandSignature[] = [
  {
    name: 'Google',
...
export class VisualPhishingDetector extends EventEmitter {
  private config: Required<VisualPhishingConfig>;
  private browser: Browser | null = null;
  private knownBrands: Map<string, BrandSignature> = new Map();
  private analyses: PhishingAnalysis[] = [];
  ...
  async analyzeUrl(url: string): Promise<PhishingAnalysis> {
  ...
  async scanUrls(urls: string[]): Promise<PhishingReport> {
    const startTime = new Date();
    this.emit('scan:start', { urls: urls.length });

    const analyses: PhishingAnalysis[] = [];
    ...
    const highRiskPages = analyses.filter(a => 
      a.riskAssessment.level === 'high' || a.riskAssessment.level === 'critical'
    );
   ...
   */
  // Complexity: O(1)
  private analyzeDomain(url: string, _page: Page): PhishingAnalysis['domainInfo'] {
  ...
  private detectTyposquatting(domain: string): boolean {
    ...
    for (const pattern of typosquattingPatterns) {
      if (pattern.test(domainLower)) {
        ...
        const isLegitimate = this.config.knownBrands.some(brand => 
          brand.domains.includes(domainLower)
        );
        if (!isLegitimate) {
          return true;
        }
   ...
   */
  // Complexity: O(N*M) — nested iteration detected
  private async analyzePageContent(page: Page): Promise<{
    loginFormDetected: boolean;
    passwordFieldCount: number;
    suspiciousElements: SuspiciousElement[];
    pageTitle: string;
    ...
    return {
      ...result,
      suspiciousElements: result.suspiciousElements as SuspiciousElement[],
    };
  }
  ...
  private async analyzeWithGeminiVision(
    screenshotPath: string,
    url: string,
    pageAnalysis: { pageTitle: string; pageText: string; loginFormDetected: boolean }
  ): Promise<PhishingAnalysis['visualAnalysis']> {
...
Respond in this JSON format:
{
  ...
  "overallPhishingLikelihood": 85
      ...
      if (jsonMatch) {
        ...
        return {
          ...
          suspiciousElements: (parsed.suspiciousVisualElements ?? []).map((desc: string) => ({
            type: 'credential_harvester' as const,
            selector: 'visual',
            description: desc,
  ...
  private heuristicVisualAnalysis(
    url: string,
    pageAnalysis: { pageTitle: string; pageText: string; loginFormDetected: boolean }
  ): PhishingAnalysis['visualAnalysis'] {
    ...
    for (const brand of this.config.knownBrands) {
      ...
      const keywordMatches = brand.keywords.filter(kw => 
        titleLower.includes(kw.toLowerCase()) || textLower.includes(kw.toLowerCase())
      ).length;
      ...
      if (keywordMatches > 0 || titleLower.includes(brandLower) || textLower.includes(brandLower)) {
        ...
        if (!isLegitDomain && confidence > 50) {
          ...
          mismatchDetails.push(
            `Page contains ${brand.name} branding but URL "${new URL(url).hostname}" is not a legitimate ${brand.name} domain`
          );
        }
      }
    }
  ...
  private calculateRiskScore(
    domainInfo: PhishingAnalysis['domainInfo'],
    visualAnalysis: PhishingAnalysis['visualAnalysis'],
    pageAnalysis: { suspiciousElements: SuspiciousElement[] }
  ): PhishingAnalysis['riskAssessment'] {
    ...
    let level: PhishingAnalysis['riskAssessment']['level'];
   ...
   */
  // Complexity: O(N) — linear iteration
  private calculateOverallRisk(analyses: PhishingAnalysis[]): number {
  ...
  private async generateAISummary(
    ...
    domainInfo: PhishingAnalysis['domainInfo'],
    visualAnalysis: PhishingAnalysis['visualAnalysis'],
    riskAssessment: PhishingAnalysis['riskAssessment']
  ): Promise<string> {
...
`.trim();
   ...
   */
  // Complexity: O(N) — linear iteration
  private generateRecommendations(analyses: PhishingAnalysis[]): string[] {
  ...
  generateReport(target: string, startTime: Date): PhishingReport {
    const endTime = new Date();
    const highRiskPages = this.analyses.filter(a => 
      a.riskAssessment.level === 'high' || a.riskAssessment.level === 'critical'
    );
