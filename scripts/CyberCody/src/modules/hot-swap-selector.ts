/**
 * hot-swap-selector — Qantum Module
 * @module hot-swap-selector
 * @path scripts/CyberCody/src/modules/hot-swap-selector.ts
...
export class HotSwapSelectorEngine extends EventEmitter {
  private config: Required<HotSwapConfig>;
  private browser: Browser | null = null;
  private memory: SelectorMemory;
  private activeObservers: Map<string, boolean> = new Map();
   ...
   */
  // Complexity: O(N*M) — nested iteration detected
  async captureFingerprint(page: Page, selector: string): Promise<SelectorFingerprint | null> {
    try {
      ...
      const fingerprint = await page.evaluate((sel) => {
        ...
        // Get parent chain
        const getParentChain = (element: Element): string[] => {
          const chain: string[] = [];
        ...
        // Get sibling context
        const getSiblingContext = (element: Element) => {
          const parent = element.parentElement;
          if (!parent) return { index: 0, totalSiblings: 1 };
      ...
      const fullFingerprint: SelectorFingerprint = {
  ...
  private generateVisualHash(fingerprint: Partial<SelectorFingerprint>): string {
    const hashInput = [
      fingerprint.tagName,
  ...
  async verifySelector(page: Page, selector: string): Promise<{ valid: boolean; mutation?: SelectorMutation }> {
  ...
  private detectMutation(original: SelectorFingerprint, current: SelectorFingerprint): SelectorMutation | null {
    const changes: string[] = [];
    let mutationType: SelectorMutation['mutationType'] = 'attribute_change';
    ...
    let confidence = 100;
    if (changes.includes('position')) confidence -= 15;
    if (changes.includes('size')) confidence -= 10;
    if (changes.includes('text')) confidence -= 20;
    if (changes.includes('style')) confidence -= 10;
    if (changes.includes('parent')) confidence -= 30;
    if (changes.includes('attributes')) confidence -= 15;
   ...
   */
  // Complexity: O(N*M) — nested iteration detected
  async hotSwap(page: Page, selector: string): Promise<HotSwapResult> {
    ...
    for (const strategy of strategies) {
      try {
        ...
        if (result && result.confidence >= this.config.minSwapConfidence) {
          ...
          const swapResult: HotSwapResult = {
            success: true,
            originalSelector: selector,
    ...
    const failResult: HotSwapResult = {
      success: false,
      originalSelector: selector,
      confidence: 0,
      mutation: verification.mutation,
   ...
   */
  // Complexity: O(1)
  private async findByCssPath(page: Page, fingerprint: SelectorFingerprint): Promise<{ selector: string; confidence: number } | null> {
    // SAFETY: async operation — wrap in try-catch for production resilience
   ...
   */
  // Complexity: O(1)
  private async findByXpath(page: Page, fingerprint: SelectorFingerprint): Promise<{ selector: string; confidence: number } | null> {
    // SAFETY: async operation — wrap in try-catch for production resilience
   ...
   */
  // Complexity: O(1)
  private async findByTextContent(page: Page, fingerprint: SelectorFingerprint): Promise<{ selector: string; confidence: number } | null> {
    if (!fingerprint.textContent || fingerprint.textContent.length < 3) return null;
  ...
  private async findByVisualSimilarity(page: Page, fingerprint: SelectorFingerprint): Promise<{ selector: string; confidence: number } | null> {
    // SAFETY: async operation — wrap in try-catch for production resilience
    ...
    }, { boundingBox: fingerprint.boundingBox } as { boundingBox: typeof fingerprint.boundingBox });
    ...
    if (candidates.length > 0) {
      ...
      if (best) {
        ...
        else selector += `:nth-of-type(${best.index + 1})`;
   ...
   */
  // Complexity: O(N) — linear iteration
  private async findByAriaAttributes(page: Page, fingerprint: SelectorFingerprint): Promise<{ selector: string; confidence: number } | null> {
    const ariaKeys = Object.keys(fingerprint.ariaAttributes);
    if (ariaKeys.length === 0) return null;
   ...
   */
  // Complexity: O(N) — linear iteration
  private async findByDataAttributes(page: Page, fingerprint: SelectorFingerprint): Promise<{ selector: string; confidence: number } | null> {
   ...
   */
  // Complexity: O(1) — hash/map lookup
  private async findByParentContext(page: Page, fingerprint: SelectorFingerprint): Promise<{ selector: string; confidence: number } | null> {
    if (fingerprint.parentChain.length === 0) return null;
   ...
   */
  // Complexity: O(1) — amortized
  private async findByGeminiVision(page: Page, fingerprint: SelectorFingerprint): Promise<{ selector: string; confidence: number } | null> {
    if (!this.geminiModel || !this.config.enableVisualMatching) return null;
...
Return ONLY a CSS selector that would uniquely identify this element. If you cannot find it, return "NOT_FOUND".`;
   ...
   */
  // Complexity: O(N*M) — nested iteration detected
  async startMutationObserver(page: Page, selectors: string[]): Promise<void> {
    ...
    const verifyLoop = async () => {
      for (const selector of selectors) {
        if (!this.activeObservers.get(selector)) continue;
  ...
  private saveMemory(): void {
    try {
      ...
      // Complexity: O(1)
      writeFileSync(this.config.memoryFilePath, JSON.stringify(data, null, 2));
    } catch (error) {
  ...
  private loadMemory(): void {
    try {
      if (existsSync(this.config.memoryFilePath)) {
        const data = JSON.parse(readFileSync(this.config.memoryFilePath, 'utf-8'));
        ...
        this.memory.lastVerified = new Map(
          (data.lastVerified || []).map(([k, v]: [string, string]) => [k, new Date(v)])
        );
