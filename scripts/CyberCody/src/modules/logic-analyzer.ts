/**
 * logic-analyzer — Qantum Module
 * @module logic-analyzer
 * @path scripts/CyberCody/src/modules/logic-analyzer.ts
...
export type SensitiveCategory = 
  ...
  | 'health'        // Medical records, health data (HIPAA)
  | 'location'      // GPS, addresses, geolocation
  | 'internal'      // Internal IDs, debug info, stack traces
  | 'business';     // Proprietary data, pricing, contracts
...
export type LogicFlawType = 
  ...
  | 'debug_info_leak'             // Stack traces, debug flags
  | 'internal_id_exposure'        // Internal system IDs exposed
  | 'inconsistent_authorization'  // Different data for same auth level
  | 'mass_assignment'             // Accepts more fields than documented
  | 'rate_limit_bypass'           // No rate limiting detected
  | 'predictable_identifiers'     // Sequential/guessable IDs
...
const SENSITIVE_PATTERNS: Record<SensitiveCategory, Array<{ name: string; pattern: RegExp; severity: DataExposureSeverity; compliance: ComplianceTag[] }>> = {
  pii: [
...
function analyzeLocally(responseBody: string, endpoint: string): Omit<LogicAnalysisResult, 'aiInsights'> {
  const exposures: DataExposure[] = [];
  const logicFlaws: LogicFlaw[] = [];
  let maxSeverity: DataExposureSeverity = 'info';
  
  const severityOrder: Record<DataExposureSeverity, number> = {
    info: 0, low: 1, medium: 2, high: 3, critical: 4
  };

  // Scan for sensitive data patterns
  for (const [category, patterns] of Object.entries(SENSITIVE_PATTERNS) as [SensitiveCategory, typeof SENSITIVE_PATTERNS['pii']][]) {
...
function getRemediation(category: SensitiveCategory, _field: string): string {
  const remediations: Record<SensitiveCategory, string> = {
...
export class LogicAnalyzer extends EventEmitter {
  private config: Required<LogicAnalyzerConfig>;
  private results: LogicAnalysisResult[] = [];
  ...
  async analyzeAPIMap(apiMap: APIMap): Promise<LogicAnalysisReport> {
    ...
    for (const request of apiMap.requests) {
      if (!request.response?.body) continue;
  ...
  async analyzeResponse(
    ...
    let aiInsights: AIAnalysis | undefined;
  ...
  private generateReport(target: string, _startTime: Date): LogicAnalysisReport {
    ...
    const complianceViolations: Record<ComplianceTag, number> = {
   ...
   */
  // Complexity: O(N*M) — nested iteration detected
  private printReport(report: LogicAnalysisReport): void {
    console.log(`
╔══════════════════════════════════════════════════════════════════════════════╗
