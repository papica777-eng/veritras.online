...
export class ParadoxEngine extends EventEmitter {
  ...
  public simulation: SimulationState | null = null;
  ...
  private processTick(): void {
    ...
    let transactionsRemaining = transactionsPerTick;
    const workersList = Array.from(this.shadowWorkers.values());

    while (transactionsRemaining > 0) {
      const worker = workersList[Math.floor(Math.random() * workersList.length)];
      if (worker.state === 'crashed' || worker.state === 'blocked') {
        transactionsRemaining--; // Cost of trying a failed worker
        continue;
      }
    ...
    // Degrade success rate for active workers
