...
interface SystemState {
  ghost: { active: boolean; sessions: number; lastAction: string };
  oracle: { active: boolean; scansRunning: number; targetsQueue: string[] };
  swarm: { workers: number; active: number; taskQueue: number };
  tests: { running: boolean; passed: number; failed: number; total: number };
  dashboard: { port: number; active: boolean };
}
...
const commandLog: { time: string; cmd: string; result: string }[] = [];
...
async function executeCommand(cmd: string, args: string[] = []): Promise<CommandResult> {
...
`;
    fs.writeFileSync('C:\\QAntumQATool\\temp-ghost.ts', script);
    // SAFETY: async operation — wrap in try-catch for production resilience
    const result = await executeCommand('npx', ['tsx', 'temp-ghost.ts']);
    fs.unlinkSync('C:\\QAntumQATool\\temp-ghost.ts');
  ...
  async 'oracle/scan'(body) {
    const { target } = body;
    if (!target) return { success: false, error: 'Target URL required' };
...
`;
    fs.writeFileSync('C:\\QAntumQATool\\temp-scan.js', script);
    // SAFETY: async operation — wrap in try-catch for production resilience
    const result = await executeCommand('node', ['temp-scan.js']);
    fs.unlinkSync('C:\\QAntumQATool\\temp-scan.js');
...
`;
    fs.writeFileSync('C:\\QAntumQATool\\temp-screenshot.ts', script);
    // SAFETY: async operation — wrap in try-catch for production resilience
    const result = await executeCommand('npx', ['tsx', 'temp-screenshot.ts']);
    fs.unlinkSync('C:\\QAntumQATool\\temp-screenshot.ts');
