/**
 * Bola-Shadow — Qantum Module
 * @module Bola-Shadow
 * @path scripts/tests/Bola-Shadow.ts
 * @auto-documented BrutalDocEngine v2.1
 */


// ╔══════════════════════════════════════════════════════════════════════════════╗
// ║  CYBERCODY v1.1.0 - BOLA TESTER (SHADOW EXECUTION)                           ║
...
import { generateUUID } from '../scripts/utils/crypto.ts';
...
import type {
    ...
    CapturedRequest,
    APIMapStatistics
} from '../modules/api-interceptor.ts';
...
function buildUrl(
    ...
    if (paramType === 'query') {
        ...
        return url.pathname + url.search; // Warning: this drops base URL if not part of endpoint
    }
...
export class BOLATester extends EventEmitter {
    ...
    private runWorker(data: BOLAWorkerData): Promise<BOLAWorkerResult> {
        return new Promise((resolve, reject) => {
            // Use __filename equivalent for TypeScript execution in ES modules/TSX
...
// ═══════════════════════════════════════════════════════════════════════════════
// 🚀 RUNNER EXECUTION (SHADOW MODE)
// ═══════════════════════════════════════════════════════════════════════════════

// Only run if main thread (and executed directly)
if (isMainThread) {
    // Check if we assume we are running as an entry point. 
    // In worker_threads with tsx, the worker also runs the file, but isMainThread is false.

    (async () => {
        // mock data
        const mockMap: APIMap = {
            target: 'http://localhost:3000',
            scanStarted: new Date(),
            endpoints: new Map(),
            requests: [],
            authTokens: [
                { type: 'bearer', token: 'mock-valid-token', location: 'header' },
                { type: 'bearer', token: 'mock-admin-token', location: 'header' }
            ],
            sensitiveEndpoints: [],
            bolaTargets: [
                {
                    endpoint: 'http://jsonplaceholder.typicode.com/users/1',
                    method: 'GET',
                    parameter: 'id',
                    parameterType: 'path',
                    originalValue: '1',
        ...
        console.log('🚀 Launching Shadow BOLA Tester...');
        const tester = new BOLATester({
            apiMap: mockMap,
            variationsPerTarget: 5, // Keep it fast
            workerCount: 2
        });
