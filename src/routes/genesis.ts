...
import {
  ...
  CausalityType,
  ModalSystem
} from '../engines/OntoGenerator';
import { phenomenonWeaver, PotentialType, ObservationType } from '../engines/PhenomenonWeaver';
...
export const genesisRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/reweaveCausality', async (request, reply) => {
    try {
      ...
      const rewoven = ontoGenerator.reweaveCausality(body.causalWeb as any, body.modifications as any);
  ...
  app.post('/observe', async (request, reply) => {
    try {
      ...
      const result = phenomenonWeaver.observeReality(body.realityId as string, body.observationType as ObservationType);
