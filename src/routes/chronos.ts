...
// ═══════════════════════════════════════════════════════════════════════════════
// CATUSKOTI LOGIC CORE
// ═══════════════════════════════════════════════════════════════════════════════

type CatuskotiState = 'TRUE' | 'FALSE' | 'PARADOX' | 'TRANSCENDENT';

interface CatuskotiSignal {
    state: CatuskotiState;
    confidence: number;       // 0-100
    reasoning: string;
    entropy: number;          // Shannon entropy
    bidPressure: number;      // Order book bid/ask ratio
    manifoldCurvature: number; // OBI Gaussian curvature
}

interface SoulKarma {
    level: number;
    phase: 'AWAKEN' | 'CONSUME' | 'PROTECT' | 'EVOLVE' | 'TRANSMUTE';
    totalDecisions: number;
    correctDecisions: number;
    accuracy: number;
}

interface RetrocausalProjection {
    timeframe: string;
...
interface ChronosPrediction {
    pair: string;
    timestamp: number;
    currentPrice: number;
    signal: CatuskotiSignal;
    soulKarma: SoulKarma;
    retrocausal: RetrocausalProjection[];
    orderBook: {
        bidVolume: number;
        askVolume: number;
        imbalance: number;
...
function evaluateCatuskoti(
    bidPressure: number,
    manifoldCurvature: number,
    shortTermForecast: number,
...
function generateRetrocausalProjections(
    currentPrice: number,
    bidPressure: number,
    curvature: number
): RetrocausalProjection[] {
    ...
    return timeframes.map((tf, i) => {
        ...
        let state: CatuskotiState;
        if (paradoxDetected) state = 'PARADOX';
        else if (predictedChange > 0) state = 'TRUE';
        else if (predictedChange < 0) state = 'FALSE';
...
const soulKarmaStore: Map<string, SoulKarma> = new Map();

// Complexity: O(1)
function getOrCreateKarma(tenantId: string): SoulKarma {
...
const PHASES: SoulKarma['phase'][] = ['AWAKEN', 'CONSUME', 'PROTECT', 'EVOLVE', 'TRANSMUTE'];

function evolveKarma(karma: SoulKarma): void {
...
async function fetchBinancePrice(pair: string): Promise<number> {
    try {
...
async function fetchOrderBook(pair: string, limit = 20): Promise<BinanceOrderBook> {
    try {
        const res = await fetch(`https://api.binance.com/api/v3/depth?symbol=${pair}&limit=${limit}`);
        return await res.json() as BinanceOrderBook;
    } catch {
        return { bids: [], asks: [] };
    }
}

function computeOrderBookMetrics(book: BinanceOrderBook) {
...
// ═══════════════════════════════════════════════════════════════════════════════
// PLAN LIMITS FOR CHRONOS
// ═══════════════════════════════════════════════════════════════════════════════

interface ChronosPlanLimits {
    predictionsPerDay: number;
    pairs: string[];
    retrocausalEnabled: boolean;
    orderBookLevels: number;
    soulKarmaAccess: boolean;
}

const CHRONOS_PLANS: Record<string, ChronosPlanLimits> = {
    CHRONOS_STARTER: {
...
// Usage tracker
const chronosUsage: Map<string, { count: number; resetAt: number }> = new Map();

function checkChronosLimit(tenantId: string, plan: string): { allowed: boolean; remaining: number } {
    const limits = CHRONOS_PLANS[plan] || CHRONOS_PLANS.CHRONOS_STARTER;
    if (limits.predictionsPerDay === -1) return { allowed: true, remaining: Infinity };
...
export const chronosRoutes: FastifyPluginAsync = async (app) => {
     ...
     */
    app.post('/predict', { preHandler: requireAuth }, async (request, reply) => {
        // SAFETY: async operation — wrap in try-catch for production resilience
        const tenant = await getTenant(request);

        const schema = z.object({
            pair: z.string().min(3).max(20).default('ETHEUR'),
            includeRetrocausal: z.boolean().default(true),
            orderBookLevels: z.number().min(5).max(50).default(20),
        });

        const body = schema.parse(request.body);
        const plan = (tenant as any).chronosPlan || 'CHRONOS_STARTER';
        const limits = CHRONOS_PLANS[plan] || CHRONOS_PLANS.CHRONOS_STARTER;
        ...
        // Catuskoti evaluation
        const signal = evaluateCatuskoti(
            obMetrics.imbalance,
            obMetrics.curvature,
            shortTermForecast,
            longTermForecast
        ...
        const prediction: ChronosPrediction = {
            pair: body.pair,
            timestamp: Date.now(),
            currentPrice: price,
            signal,
     ...
     */
    app.get('/karma', { preHandler: requireAuth }, async (request) => {
        // SAFETY: async operation — wrap in try-catch for production resilience
        const tenant = await getTenant(request);
        const karma = getOrCreateKarma(tenant.id);
