 ...
 * @module Reality
 * @path src/core/departments/Reality.ts
...
export class RealityDepartment extends Department {
  ...
  // Complexity: O(1) — world state initialization
  ...
  // Complexity: O(S) where S = active simulations to clear
   ...
   */
  // Complexity: O(S) where S = simulations (Map.values conversion)
   ...
   */
  // Complexity: O(K) where K = keys in update object (spread merge)
   ...
   */
  // Complexity: O(1) — render command delegation
  ...
  public async sync(): Promise<void> { console.log('[Reality] Syncing...'); }
}
