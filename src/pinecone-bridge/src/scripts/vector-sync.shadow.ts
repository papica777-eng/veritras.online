...
import { getEmbeddingEngine } from '../EmbeddingEngine.js';
...
export class VectorSyncEngine extends EventEmitter {
                        ...
                        for (let i = 0; i < allIds.length; i += 100) {
                            ...
                            for (const id of batchIds) {
                                ...
                                if (record?.metadata) {
                                    vectors.set(id, {
                                        id,
                                        filePath: (record.metadata.filePath as string) || id,
    ...
    async sync(options: { force?: boolean; addOnly?: boolean; deleteOrphans?: boolean } = {}): Promise<SyncResult> {
        ...
        if (delta.toAdd.length > 0 || delta.toUpdate.length > 0) {
            ...
            this.embedEngine = await getEmbeddingEngine();
