...
export class PlannerAgent extends BaseAgent {
  ...
  protected async handleMessage(message: SwarmMessage): Promise<SwarmMessage | null> {
    switch (message.type) {
      case 'task': {
        // Planning request
        const payload = message.payload as { goal: string; context?: Record<string, unknown> };
      ...
      case 'feedback': {
        ...
        const payload = message.payload as { planId: string; feedback: string };
  ...
  private decomposeGoal(
    ...
    if (goalLower.includes('login') || goalLower.includes('authenticate')) {
      tasks.push(
        this.createTask('navigate', (context?.url as string) || 'login page', { action: 'navigate' }, tid, 'high'),
      ...
      tasks.push(
        this.createTask('navigate', (context?.url as string) || 'target page', { action: 'navigate' }, tid, 'high'),
        this.createTask('extract', (context?.selector as string) || 'data elements', { format: context?.format || 'json' }, tid)
      );
    } else if (goalLower.includes('fill') && goalLower.includes('form')) {
      tasks.push(
        this.createTask('navigate', (context?.url as string) || 'form page', { action: 'navigate' }, tid, 'high'),
        ...this.createFormTasks((context?.fields as Record<string, unknown>) || {}, tid),
      ...
      tasks.push(
        this.createTask('navigate', (context?.url as string) || 'test page', { action: 'navigate' }, tid, 'high'),
