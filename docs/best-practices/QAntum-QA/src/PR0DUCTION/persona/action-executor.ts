...
export class ActionExecutor extends EventEmitter {
    ...
    private async executeClick(action: Action): Promise<{
        ...
        const getElementInfo = (selector: unknown): { x: number; y: number; width: number; height: number } | null => {
        ...
        const elementInfo = await this.page.evaluate(
            getElementInfo as (...args: unknown[]) => { x: number; y: number; width: number; height: number } | null,
            action.selector
        );
    ...
    private async executeScroll(action: Action): Promise<void> {
        ...
        const scrollFn = (amount: unknown): void => {
            window.scrollBy({ top: amount as number, behavior: 'smooth' });
        };
        await this.page.evaluate(
            scrollFn as (...args: unknown[]) => void,
            scrollAmount
        );
    }
