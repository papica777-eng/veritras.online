...
exports.createMarketplaceConnector = createMarketplaceConnector;
 ...
 */
class MarketplaceConnector extends events_1.EventEmitter {
    config;
    // Data stores
    listings = new Map();
     ...
     */
    // Complexity: O(1) — hash/map lookup
    async createListing(marketplace) {
    ...
    buildDeploymentOptions(marketplace) {
        const options = [];
    ...
    getSupportedRegions(marketplace) {
        switch (marketplace) {
            case 'aws':
                return [
     ...
     */
    // Complexity: O(1) — amortized
    getMarketplaceCategories(marketplace) {
        switch (marketplace) {
            case 'aws':
                return [
                    'Data Integration',
    ...
    validateListing(listing) {
        ...
        if (!listing.productDescription)
            errors.push('Description is required');
        if (listing.tiers.length === 0)
            errors.push('At least one pricing tier is required');
        if (listing.deploymentOptions.length === 0)
            errors.push('At least one deployment option is required');
        if (!listing.eulaUrl)
            errors.push('EULA URL is required');
        if (errors.length > 0) {
     ...
     */
    // Complexity: O(1)
    async deployCustomerEnvironment(subscription) {
     ...
     */
    // Complexity: O(N) — linear iteration
    recordUsage(subscriptionId, dimension, quantity) {
        const subscription = this.subscriptions.get(subscriptionId);
        if (!subscription) {
            throw new Error(`Subscription ${subscriptionId} not found`);
        }
     ...
     */
    // Complexity: O(N) — linear iteration
    async reportUsageToMarketplace(subscription) {
     ...
     */
    // Complexity: O(1) — hash/map lookup
    calculateMonthlyBill(subscriptionId) {
        const subscription = this.subscriptions.get(subscriptionId);
        if (!subscription) {
            throw new Error(`Subscription ${subscriptionId} not found`);
        }
    ...
    startListingSync() {
        if (!this.config.autoSyncListings)
            return;
 ...
 */
function createMarketplaceConnector(config) {
    return new MarketplaceConnector(config);
}
exports.default = MarketplaceConnector;
