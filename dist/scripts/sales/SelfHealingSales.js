...
exports.SelfHealingSales = void 0;
exports.createSelfHealingSales = createSelfHealingSales;
 ...
 */
class SelfHealingSales extends events_1.EventEmitter {
    config;
    reports = new Map();
    offers = new Map();
    conversions = [];
     ...
     */
    // Complexity: O(N) — linear iteration
    detectAccessibilityIssues(results) {
        const issues = [];
     ...
     */
    // Complexity: O(N)
    async generateReport(clientKeyId, organizationId, crawlJobId, targetUrl, issues, industry) {
        ...
        const valueProposition = this.generateValueProposition(issues, healthScore, benchmarkWithPercentile);
        // Generate offers
     ...
     */
    // Complexity: O(N) — linear iteration
    calculateOpportunityScore(issues) {
        let score = 0;
        for (const issue of issues) {
            // Quick wins have higher opportunity
            if (issue.canBeAutofixed)
                score += 10;
            if (issue.fixComplexity === 'trivial')
                score += 8;
            if (issue.fixComplexity === 'simple')
                score += 5;
     ...
     */
    // Complexity: O(1)
    calculatePercentile(healthScore, benchmark) {
    ...
    generatePrioritizedActions(issues) {
        const actions = [];
     ...
     */
    // Complexity: O(N) — linear iteration
    generateValueProposition(issues, healthScore, benchmark) {
     ...
     */
    // Complexity: O(N) — linear iteration
    generateOffers(issues, clientKeyId) {
        const offers = [];
        const now = new Date();
        ...
        if (autofixableIssues.length > 0) {
            const autofixTotal = autofixableIssues.reduce((sum, i) => sum + (i.autofixPrice || this.config.autofixBasePricePerIssue), 0);
        ...
        // Test suite offer
        const testSuiteTotal = issues.reduce((sum, i) => sum + (i.testSuitePrice || this.config.testSuiteBasePricePerIssue), 0);
        ...
        if (autofixableIssues.length > 0) {
            const bundleTotal = (autofixableIssues.reduce((sum, i) => sum + (i.autofixPrice || 99), 0)) +
                testSuiteTotal +
     ...
     */
    // Complexity: O(1) — hash/map lookup
    trackOfferEvent(offerId, clientKeyId, eventType, revenue) {
...
exports.SelfHealingSales = SelfHealingSales;
// ═══════════════════════════════════════════════════════════════════════════
// FACTORY EXPORT
// ═══════════════════════════════════════════════════════════════════════════
/**
 ...
 */
function createSelfHealingSales(config) {
    return new SelfHealingSales(config);
}
exports.default = SelfHealingSales;
