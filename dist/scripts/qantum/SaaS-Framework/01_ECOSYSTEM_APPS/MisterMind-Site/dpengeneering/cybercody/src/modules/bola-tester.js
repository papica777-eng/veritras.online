 ...
 * @module bola-tester
 * @path scripts/qantum/SaaS-Framework/01_ECOSYSTEM_APPS/MisterMind-Site/dpengeneering/cybercody/src/modules/bola-tester.ts
 * @auto-documented BrutalDocEngine v2.1
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.BOLATester = void 0;
// ╔══════════════════════════════════════════════════════════════════════════════╗
 ...
 */
function generateIDMutations(originalValue, customIds = []) {
    const mutations = [];
    // Numeric IDs
    if (/^\d+$/.test(originalValue)) {
        const numValue = parseInt(originalValue, 10);
        mutations.push({ type: 'increment', value: String(numValue + 1), description: 'ID + 1' }, { type: 'decrement', value: String(numValue - 1), description: 'ID - 1' }, { type: 'increment', value: String(numValue + 10), description: 'ID + 10' }, { type: 'increment', value: String(numValue + 100), description: 'ID + 100' }, { type: 'zero', value: '0', description: 'Zero ID' }, { type: 'zero', value: '1', description: 'First ID' }, { type: 'negative', value: '-1', description: 'Negative ID' }, { type: 'max', value: '999999999', description: 'Large ID' }, { type: 'random', value: String(Math.floor(Math.random() * 10000)), description: 'Random ID' });
    }
    // UUID
    ...
    // MongoDB ObjectId
        ...
        mutations.push({ type: 'increment', value: incrementHex(originalValue), description: 'ObjectId +1' }, { type: 'decrement', value: decrementHex(originalValue), description: 'ObjectId -1' }, { type: 'zero', value: '000000000000000000000001', description: 'First ObjectId' }, { type: 'random', value: generateRandomObjectId(), description: 'Random ObjectId' });
    }
    // String/slug
    else {
        mutations.push({ type: 'custom', value: 'admin', description: 'Admin user' }, { type: 'custom', value: 'test', description: 'Test user' }, { type: 'custom', value: 'user', description: 'Generic user' }, { type: 'custom', value: '1', description: 'Numeric 1' });
    }
    // Add custom IDs
...
function detectSensitiveFields(body) {
    const found = [];
...
if (!worker_threads_1.isMainThread && worker_threads_1.parentPort) {
    ...
    (async () => {
        ...
        for (const mutation of data.mutations) {
            try {
                const testedUrl = buildUrl(data.target.endpoint, data.target.parameter, mutation.value, data.target.parameterType);
                const testedResponse = await makeRequest(testedUrl, data.target.method, data.authToken, data.timeoutMs);
                // Analyze vulnerability
                const vulnerability = analyzeVulnerability(baselineResponse, testedResponse, data.target, mutation);
                results.push({
                    target: data.target,
...
function buildUrl(endpoint, parameter, value, paramType) {
    if (paramType === 'query') {
...
async function makeRequest(url, method, auth, timeout = 10000) {
...
function analyzeVulnerability(baseline, tested, target, mutation) {
    const evidence = [];
    let detected = false;
    let confidence = 'low';
    let type = 'none';
 ...
 */
class BOLATester extends events_1.EventEmitter {
    config;
    results = [];
    constructor(config) {
        super();
    ...
    generateReport(startTime) {
        ...
        const confirmedBOLA = vulnerabilities.filter(v => v.vulnerability.confidence === 'confirmed' || v.vulnerability.confidence === 'high');
        const potentialBOLA = vulnerabilities.filter(v => v.vulnerability.confidence === 'medium' || v.vulnerability.confidence === 'low');
...
║ RECOMMENDATIONS:                                                             ║`);
    ...
    getConfirmedVulnerabilities() {
        return this.results.filter(r => r.vulnerability.detected &&
...
exports.BOLATester = BOLATester;
exports.default = BOLATester;
