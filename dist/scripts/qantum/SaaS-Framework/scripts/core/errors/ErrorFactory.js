...
exports.ERROR_METADATA = exports.ErrorAggregator = exports.QAntumError = exports.ErrorSeverity = exports.ErrorCategory = exports.ErrorCodes = void 0;
...
(function (ErrorCategory) {
    ...
    ErrorCategory["ORACLE"] = "ORACLE";
    ErrorCategory["SWARM"] = "SWARM";
...
(function (ErrorSeverity) {
    ...
    ErrorSeverity[ErrorSeverity["FATAL"] = 5] = "FATAL";
...
exports.ERROR_METADATA = ERROR_METADATA;
// ═══════════════════════════════════════════════════════════════════════════════
// QANTUM ERROR CLASS
// ═══════════════════════════════════════════════════════════════════════════════
/**
...
class QAntumError extends Error {
    ...
    withContext(additionalContext) {
        return new QAntumError(this.code, { ...this.context, ...additionalContext }, this.causeChain[0]);
    }
}
 ...
 */
function networkError(url, originalError) {
    return new QAntumError(exports.ErrorCodes.NETWORK_ERROR, { url }, originalError);
}
/**
 * Бързо създаване на timeout грешка
 */
function timeoutError(operation, timeoutMs, originalError) {
    return new QAntumError(exports.ErrorCodes.TIMEOUT, { operation, timeoutMs }, originalError);
}
/**
 * Бързо създаване на auth грешка
 */
function authError(reason, originalError) {
    return new QAntumError(exports.ErrorCodes.AUTH_FAILED, { reason }, originalError);
}
/**
 * Бързо създаване на browser грешка
 */
function browserError(code, selector, originalError) {
    return new QAntumError(code, { selector }, originalError);
}
/**
 * Wrap функция с error handling
 */
function wrapWithErrorHandling(fn, defaultErrorCode = exports.ErrorCodes.UNKNOWN) {
    return ((...args) => {
        try {
            ...
            if (result instanceof Promise) {
                return result.catch(error => {
                    ...
                    throw new QAntumError(defaultErrorCode, { args }, error);
                });
            }
            return result;
        }
...
class ErrorAggregator {
    ...
    // Complexity: O(N) — linear iteration
    getMostSevere() {
        return this.errors.reduce((max, current) => (current.severity > (max?.severity ?? -1)) ? current : max, undefined);
    }
    // Complexity: O(N) — linear iteration
    throwIfErrors() {
        if (this.hasErrors()) {
