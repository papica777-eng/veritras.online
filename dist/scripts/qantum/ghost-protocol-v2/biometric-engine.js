...
class BiometricEngine {
    ...
    avgTypingWPM = 60; // Words per minute
    constructor(variance = 0.15) {
    ...
    generateBezierPath(start, end, steps = 50) {
        const path = [];
    ...
    generateScrollPattern(totalDistance) {
        const segments = [];
        let remaining = totalDistance;
        let totalDuration = 0;
        while (remaining > 0) {
            // Humans scroll in chunks, not continuous motion
            const chunkSize = Math.min(remaining, 100 + Math.random() * 300 // 100-400px chunks
            );
    ...
    calculateBehaviorScore() {
        ...
        if (this.mouseMovementCount > 0)
            score += 0.1;
        if (this.mouseMovementCount > 5)
            score += 0.05;
