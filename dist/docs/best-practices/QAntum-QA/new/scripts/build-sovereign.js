...
function generateSovereigntyHash(dna) {
    ...
    const secondPass = (0, crypto_1.createHash)('sha512').update(firstPass + 'QANTUM_SOVEREIGNTY_SEAL').digest('hex');
    const finalHash = (0, crypto_1.createHash)('sha512').update(secondPass + 'MISTER_MIND_FORGE').digest('hex');
    return finalHash;
}
// ═══════════════════════════════════════════════════════════════════════════════
...
function injectHardwareDNA(hash) {
    ...
    for (const targetPath of DNA_INJECTION_TARGETS) {
        ...
        const originalContent = (0, fs_1.readFileSync)(fullPath, 'utf-8');
        ...
        if (injectedContent !== originalContent) {
            (0, fs_1.writeFileSync)(fullPath, injectedContent);
...
function restoreOriginalFiles(backups) {
    ...
    for (const backup of backups) {
        (0, fs_1.writeFileSync)(backup.filePath, backup.originalContent);
...
function compileTypeScript() {
    ...
    try {
        ...
        (0, child_process_1.execSync)('npx tsc --skipLibCheck --noEmit false --outDir dist-forge 2>&1 || echo "TypeScript warnings (continuing...)"', {
            encoding: 'utf-8',
            stdio: 'inherit'
        });
...
function bundleWithEsbuild() {
    ...
    try {
        const result = (0, child_process_1.execSync)(`npx esbuild src/ide/extension.ts --bundle --outfile=dist/extension.js --platform=node --target=node18 --format=cjs --external:vscode --external:node-schedule --external:@pinecone-database/pinecone --sourcemap`, {
            encoding: 'utf-8',
            stdio: 'pipe'
        });
...
function createPackageJson() {
    ...
    (0, fs_1.writeFileSync)('dist/package.json', JSON.stringify(packageJson, null, 2));
...
function copyAssets() {
    ...
    (0, fs_1.writeFileSync)(sidebarIconPath, sidebarIconSvg);
  ...
  <line x1="75" y1="75" x2="50" y2="50" stroke="#7c3aed" stroke-width="1" opacity="0.4"/>
    ...
    (0, fs_1.writeFileSync)(qantumIconPath, qantumIconSvg);
...
function packageVsix() {
    ...
    try {
        ...
        if (!(0, fs_1.existsSync)('dist/CHANGELOG.md')) {
            (0, fs_1.writeFileSync)('dist/CHANGELOG.md', `# Changelog\n\n## ${EXTENSION_VERSION}\n- Initial sovereign release\n`);
        }
        // Create README.md if not exists
        if (!(0, fs_1.existsSync)('dist/README.md')) {
            (0, fs_1.writeFileSync)('dist/README.md', `# QAntum\n\nIdentity-Locked AI Intelligence\n\n"В QAntum не лъжем."\n`);
        }
        // Create LICENSE file
        if (!(0, fs_1.existsSync)('dist/LICENSE')) {
            (0, fs_1.writeFileSync)('dist/LICENSE', `PROPRIETARY LICENSE - DIMITAR PRODROMOV\n\nThis software is private property of Dimitar Prodromov.\nUnauthorized use, copying, or distribution is strictly prohibited.\n\n© 2026 QAntum Empire. All rights reserved.\n`);
        }
        (0, child_process_1.execSync)(`cd dist && npx vsce package --out ${vsixName} --allow-missing-repository`, {
            encoding: 'utf-8',
            stdio: 'inherit'
        });
