...
exports.CAPABILITY_STATS = exports.ALL_CAPABILITIES = exports.CATEGORIES = void 0;
exports.getCapabilitiesByCategory = getCapabilitiesByCategory;
exports.getCapabilitiesByTier = getCapabilitiesByTier;
...
function getCapabilitiesByCategory(category) {
    return exports.ALL_CAPABILITIES.filter(cap => cap.category === category);
}
function getCapabilitiesByTier(tier) {
    return exports.ALL_CAPABILITIES.filter(cap => cap.tier === tier);
}
function searchCapabilities(query) {
    const lowerQuery = query.toLowerCase();
    return exports.ALL_CAPABILITIES.filter(cap => cap.name.toLowerCase().includes(lowerQuery) ||
...
function getCapabilityById(id) {
    return exports.ALL_CAPABILITIES.find(cap => cap.id === id);
}
// Statistics
exports.CAPABILITY_STATS = {
    total: exports.ALL_CAPABILITIES.length,
    byCategory: Object.fromEntries(Object.keys(exports.CATEGORIES).map(cat => [
        cat,
        exports.ALL_CAPABILITIES.filter(c => c.category === cat).length
    ])),
    byTier: {
        free: exports.ALL_CAPABILITIES.filter(c => c.tier === 'free').length,
        pro: exports.ALL_CAPABILITIES.filter(c => c.tier === 'pro').length,
        enterprise: exports.ALL_CAPABILITIES.filter(c => c.tier === 'enterprise').length
    }
};
