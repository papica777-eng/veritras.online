...
`);
        ...
        logger_1.logger.debug(`   Duration: ${this.config.duration / 1000}s`);
        logger_1.logger.debug(`   Max Workers: ${this.config.maxWorkers}`);
        logger_1.logger.debug(`   Target CPU: ${this.config.targetCPU}%`);
        logger_1.logger.debug(`   Target Memory: ${this.config.targetMemory}%`);
        logger_1.logger.debug(`   Phases to test: ${this.config.phases.length}`);
        logger_1.logger.debug('');
        this.isRunning = true;
        ...
        try {
            // Run all phases
            logger_1.logger.debug('🚀 Starting stress test...');
            logger_1.logger.debug('');
            ...
            clearInterval(metricsInterval);
            this.isRunning = false;
            logger_1.logger.error('❌ Stress test failed:', error.message);
            throw error;
        }
    }
    /**
    ...
    displayReport(report) {
        logger_1.logger.debug('');
        logger_1.logger.debug('╔═══════════════════════════════════════════════════════════════╗');
        logger_1.logger.debug('║  📊 STRESS TEST REPORT                                        ║');
        logger_1.logger.debug('╠═══════════════════════════════════════════════════════════════╣');
        const statusIcon = report.passRate >= 95 ? '✅' : report.passRate >= 80 ? '⚠️' : '❌';
        logger_1.logger.debug(`║  ${statusIcon} Status: ${report.passRate >= 95 ? 'PASSED' : 'NEEDS ATTENTION'}`.padEnd(62) + '║');
        logger_1.logger.debug('╠═══════════════════════════════════════════════════════════════╣');
        logger_1.logger.debug(`║  Duration: ${(report.duration / 1000).toFixed(2)}s`.padEnd(62) + '║');
        logger_1.logger.debug(`║  Total Phases: ${report.totalPhases}`.padEnd(62) + '║');
        logger_1.logger.debug(`║  Passed: ${report.passedPhases} (${report.passRate.toFixed(1)}%)`.padEnd(62) + '║');
        logger_1.logger.debug(`║  Failed: ${report.failedPhases}`.padEnd(62) + '║');
        logger_1.logger.debug(`║  System Stability: ${report.systemStability.toFixed(1)}%`.padEnd(62) + '║');
        logger_1.logger.debug('╠═══════════════════════════════════════════════════════════════╣');
        logger_1.logger.debug('║  PEAK METRICS:'.padEnd(62) + '║');
        logger_1.logger.debug(`║    Max CPU: ${report.peakMetrics.maxCPU}%`.padEnd(62) + '║');
        logger_1.logger.debug(`║    Max Memory: ${report.peakMetrics.maxMemory.toFixed(1)}%`.padEnd(62) + '║');
        logger_1.logger.debug(`║    Max Workers: ${report.peakMetrics.maxWorkers}`.padEnd(62) + '║');
        logger_1.logger.debug('╠═══════════════════════════════════════════════════════════════╣');
        logger_1.logger.debug('║  RECOMMENDATIONS:'.padEnd(62) + '║');
        for (const rec of report.recommendations) {
            logger_1.logger.debug(`║    • ${rec}`.padEnd(62) + '║');
        }
        logger_1.logger.debug('╚═══════════════════════════════════════════════════════════════╝');
        // List failed phases if any
        if (report.failedPhases > 0) {
            logger_1.logger.debug('');
            logger_1.logger.debug('❌ Failed Phases:');
            for (const result of report.phaseResults.filter(r => r.status === 'failed')) {
                logger_1.logger.debug(`   • ${result.phase}: ${result.errors.join(', ')}`);
            }
        }
    }
    /**
