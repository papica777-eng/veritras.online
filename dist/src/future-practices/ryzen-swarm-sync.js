...
`);
        ...
        logger_1.logger.debug(`   🖥️ Neural Hub: ${this.localNode.cpu.model}`);
        logger_1.logger.debug(`   💾 Memory: ${Math.round(this.localNode.memory.total / 1024 / 1024 / 1024)}GB`);
        logger_1.logger.debug(`   🧠 Cores: ${this.localNode.cpu.cores}`);
        if (this.localNode.gpu) {
            logger_1.logger.debug(`   🎮 GPU: ${this.localNode.gpu.model}`);
        }
        // Start heartbeat
        this.startHeartbeat();
        ...
        logger_1.logger.debug(`   ✅ Neural Hub online and ready`);
    }
    /**
    ...
    async rebalanceTasks() {
        logger_1.logger.debug('   ⚖️ Rebalancing tasks...');
        const tasks = Array.from(this.tasks.values());
        ...
        for (const task of queuedTasks) {
            ...
            if (task.assignedTo !== optimalTarget) {
                ...
                logger_1.logger.debug(`   → Task ${task.taskId.substring(0, 8)} reassigned to ${optimalTarget}`);
            }
        }
    ...
    registerSwarmInstance(instance) {
        ...
        logger_1.logger.debug(`   ☁️ Swarm instance registered: ${fullInstance.instanceId}`);
    ...
    shutdown() {
        ...
        logger_1.logger.debug('   🛑 Ryzen-Swarm Sync shutdown');
    }
    // Complexity: O(1)
    sleep(ms) {
