...
`);
    ...
    async analyzeOpportunities() {
        ...
        if (this.config.focusAreas.length > 0) {
            this.opportunities = this.opportunities.filter(op => this.config.focusAreas.some(area => op.description.toLowerCase().includes(area) ||
...
`.trim();
    ...
    generateRecommendations() {
        ...
        const ghostOpportunities = this.opportunities.filter(o => o.description.toLowerCase().includes('ghost') ||
