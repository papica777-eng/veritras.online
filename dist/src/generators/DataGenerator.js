...
exports.DataGenerator = exports.LoremGenerator = exports.CardGenerator = exports.PhoneGenerator = exports.AddressGenerator = exports.IdentityGenerator = void 0;
// ═══════════════════════════════════════════════════════════════════════════════
// DATA POOLS
// ═══════════════════════════════════════════════════════════════════════════════
...
class IdentityGenerator {
     ...
     */
    static generateBulk(count, options) {
     ...
     * Generate email
     */
    static generateEmail(firstName, lastName, domain) {
        const first = firstName || randomFrom([...FIRST_NAMES_MALE, ...FIRST_NAMES_FEMALE]);
        const last = lastName || randomFrom(LAST_NAMES);
...
class PhoneGenerator {
     ...
     */
    static generateUK(format = 'plain') {
     ...
     */
    static generateDE(format = 'plain') {
     ...
     */
    static generate(country = 'US', format) {
        switch (country.toUpperCase()) {
            case 'US': return this.generateUS(format);
            case 'UK':
            case 'GB': return this.generateUK(format);
            case 'DE': return this.generateDE(format);
            default: return this.generateUS(format);
        }
    }
}
...
class CardGenerator {
     ...
     */
    static formatNumber(cardNumber, separator = ' ') {
...
class LoremGenerator {
    ...
    static sentence(wordCount = random(8, 15)) {
        const words = this.words(wordCount);
        return words.charAt(0).toUpperCase() + words.slice(1) + '.';
    }
    static sentences(count = 3) {
        return Array.from({ length: count }, () => this.sentence()).join(' ');
    }
    static paragraph(sentenceCount = random(3, 7)) {
        return this.sentences(sentenceCount);
    }
    static paragraphs(count = 3) {
...
class DataGenerator {
     ...
     */
    static futureDate(yearsAhead = 1) {
        const now = Date.now();
