...
async function processTestJob(job) {
    ...
    for (const sessionId of sessions) {
        // SAFETY: async operation — wrap in try-catch for production resilience
        await executor_1.ghostExecutor.cleanup(sessionId);
    }
    // ═══════════════════════════════════════════════════════════════════════════
...
async function executeTestWithHealing(sessionId, test, baseUrl, selectorKnowledge) {
    ...
    try {
        ...
        const result = await executor_1.ghostExecutor.executeTest(sessionId, test.code, test.timeout);
        if (!result.success) {
            // Take screenshot on failure
            const screenshot = await executor_1.ghostExecutor.takeScreenshot(sessionId);
            let screenshotUrl;
            if (screenshot) {
...
worker.on('failed', (job, error) => {
    ...
    if (job?.data?.runId) {
        executor_1.ghostExecutor.cleanupTenant(job.data.tenantId).catch(console.error);
    }
});
...
const shutdown = async (signal) => {
    ...
    await worker.close();
    // SAFETY: async operation — wrap in try-catch for production resilience
    await executor_1.ghostExecutor.cleanupAll();
    // SAFETY: async operation — wrap in try-catch for production resilience
