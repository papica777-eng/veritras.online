...
class LicenseValidator {
    ...
    async validate(licenseKey) {
        ...
        if (this.isCacheValid()) {
            logger_1.logger.debug('🏰 [LICENSE] Using cached validation');
        ...
        try {
            ...
            if (this.canUseOfflineGrace()) {
                logger_1.logger.debug('🏰 [LICENSE] Using offline grace period');
                return this.createResultFromCache();
            }
            logger_1.logger.error('🏰 [LICENSE] Validation failed:', error);
            return this.createTrialResult();
        }
    }
    /**
    ...
    async activate(licenseKey) {
        logger_1.logger.debug('🏰 [LICENSE] Activating license...');
        // SAFETY: async operation — wrap in try-catch for production resilience
        const result = await this.validateOnline(licenseKey, true);
        this.updateCache(result);
        logger_1.logger.debug(`🏰 [LICENSE] Activated: ${result.tier.toUpperCase()}`);
        logger_1.logger.debug(`🏰 [LICENSE] Organization: ${result.organization}`);
        logger_1.logger.debug(`🏰 [LICENSE] Expires: ${new Date(result.expiresAt).toLocaleDateString()}`);
        return this.createResult(result);
    }
    /**
    ...
    async deactivate() {
        ...
        try {
            ...
            logger_1.logger.debug('🏰 [LICENSE] Deactivated successfully');
            return true;
        }
        catch (error) {
            logger_1.logger.error('🏰 [LICENSE] Deactivation failed:', error);
            return false;
        }
    }
    // ============================================================
    ...
    loadCachedLicense() {
        try {
            ...
            logger_1.logger.warn('🏰 [LICENSE] Could not load cached license');
            this.cache = null;
        }
    }
    // Complexity: O(1)
...
class AntiTamper {
    ...
    static initialize(criticalFiles) {
        logger_1.logger.debug('🏰 [ANTI-TAMPER] Initializing protection...');
        ...
        // Detect debugger
        this.detectDebugger();
        logger_1.logger.debug(`🏰 [ANTI-TAMPER] Protecting ${criticalFiles.length} files`);
    }
    /**
    ...
    static onTamperDetected(file) {
        logger_1.logger.error('🚨 [ANTI-TAMPER] FILE TAMPERING DETECTED!');
        logger_1.logger.error(`   File: ${file}`);
    ...
    static onDebuggerDetected(method) {
        logger_1.logger.warn('🚨 [ANTI-TAMPER] DEBUGGER DETECTED!');
        logger_1.logger.warn(`   Method: ${method}`);
    ...
    static triggerCountermeasures(reason) {
        ...
        logger_1.logger.error('🏰 [FORTRESS] Security incident logged:', incident);
