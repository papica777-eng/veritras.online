...
exports.getGhostProtocol = exports.SelectorObfuscator = exports.GhostProtocol = void 0;
// ═══════════════════════════════════════════════════════════════════════════════
...
class GhostProtocol {
     ...
     */
    async executeStealthAction(sessionId, action, name = 'action') {
        const session = this.sessions.get(sessionId);
        if (!session) {
            throw new Error(`Session not found: ${sessionId}`);
        }
     ...
     */
    // Complexity: O(N) — loop
    async typeHuman(sessionId, text, typeCallback) {
        const session = this.sessions.get(sessionId);
     ...
     */
    // Complexity: O(N) — loop
    async scrollHuman(sessionId, distance, scrollCallback) {
        const session = this.sessions.get(sessionId);
        const profile = TIMING_PROFILES[session?.level || 'medium'];
        let scrolled = 0;
     ...
     */
    // Complexity: O(1) — lookup
    async clickHuman(sessionId, clickCallback) {
        const session = this.sessions.get(sessionId);
        const profile = TIMING_PROFILES[session?.level || 'medium'];
...
// SELECTOR OBFUSCATOR
// ═══════════════════════════════════════════════════════════════════════════════
class SelectorObfuscator {
    selectorMap = new Map();
    /**
     * Register alternative selectors
     */
    // Complexity: O(1) — lookup
    register(primary, alternatives) {
     ...
     */
    // Complexity: O(1)
    generateTestId(baseName) {
        const suffix = Math.random().toString(36).substr(2, 4);
        return `[data-testid="${baseName}-${suffix}"]`;
    }
}
exports.SelectorObfuscator = SelectorObfuscator;
// ═══════════════════════════════════════════════════════════════════════════════
// EXPORTS
// ═══════════════════════════════════════════════════════════════════════════════
const getGhostProtocol = () => GhostProtocol.getInstance();
exports.getGhostProtocol = getGhostProtocol;
