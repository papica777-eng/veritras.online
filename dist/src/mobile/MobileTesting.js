...
exports.DeviceFarmClient = exports.BREAKPOINTS = exports.ResponsiveTester = exports.MobileEmulator = exports.NETWORK_PRESETS = exports.DEVICES = void 0;
exports.createMobileEmulator = createMobileEmulator;
exports.createResponsiveTester = createResponsiveTester;
...
// MOBILE EMULATOR
// ═══════════════════════════════════════════════════════════════════════════════
class MobileEmulator extends events_1.EventEmitter {
    page;
    device;
    orientation;
    networkConditions;
    constructor(page, config) {
        ...
        if (typeof config.device === 'string') {
            this.device = exports.DEVICES[config.device] || exports.DEVICES['iPhone 12'];
        }
        else if (config.device) {
            this.device = config.device;
        }
        else {
            this.device = exports.DEVICES['iPhone 12'];
        }
    ...
    async setNetworkConditions(conditions) {
        if (typeof conditions === 'string') {
            this.networkConditions = exports.NETWORK_PRESETS[conditions];
        }
        else {
...
exports.MobileEmulator = MobileEmulator;
// ═══════════════════════════════════════════════════════════════════════════════
// RESPONSIVE TESTER
// ═══════════════════════════════════════════════════════════════════════════════
class ResponsiveTester extends events_1.EventEmitter {
    page;
    constructor(page) {
        super();
        this.page = page;
     ...
     */
    // Complexity: O(1)
    async testViewports(url, viewports, assertions) {
        const results = [];
     ...
     */
    // Complexity: O(1)
    async testBreakpoints(url, assertions) {
        return this.testViewports(url, exports.BREAKPOINTS.map(bp => ({ ...bp.viewport, name: bp.name })), assertions);
    }
    /**
     * Test across all devices
     */
    // Complexity: O(N) — loop
    async testDevices(url, devices, assertions) {
        const results = [];
        for (const deviceName of devices) {
            const device = exports.DEVICES[deviceName];
            if (!device)
                continue;
...
exports.ResponsiveTester = ResponsiveTester;
exports.BREAKPOINTS = [
...
// DEVICE FARM CLIENT
// ═══════════════════════════════════════════════════════════════════════════════
class DeviceFarmClient extends events_1.EventEmitter {
    provider;
    apiKey;
    baseUrl;
    constructor(config) {
...
// EXPORTS
// ═══════════════════════════════════════════════════════════════════════════════
function createMobileEmulator(page, config) {
    return new MobileEmulator(page, config);
}
function createResponsiveTester(page) {
...
exports.default = {
    ...
    NETWORK_PRESETS: exports.NETWORK_PRESETS,
