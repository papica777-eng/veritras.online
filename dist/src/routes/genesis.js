...
exports.genesisRoutes = void 0;
const zod_1 = require("zod");
const OntoGenerator_1 = require("../engines/OntoGenerator");
const PhenomenonWeaver_1 = require("../engines/PhenomenonWeaver");
const auth_1 = require("../middleware/auth");
// Store tenant-specific realities
const tenantRealities = new Map();
// Track which reality belongs to which tenant
const realityOwnership = new Map();
function getOrCreateTenantRealities(tenantId) {
    if (!tenantRealities.has(tenantId)) {
        tenantRealities.set(tenantId, new Set());
    }
    return tenantRealities.get(tenantId);
}
function registerRealityToTenant(tenantId, realityId) {
    // Complexity: O(1)
    getOrCreateTenantRealities(tenantId).add(realityId);
    realityOwnership.set(realityId, tenantId);
}
function canAccessReality(tenantId, realityId) {
...
function checkLimit(tenant, action) {
...
function incrementUsage(tenantId, action) {
...
function checkRateLimit(tenantId, maxRequests = 60, windowMs = 60000) {
    const now = Date.now();
...
// Validation schemas
const createAxiomSchema = zod_1.z.object({
    name: zod_1.z.string().min(1).max(100).optional(),
    types: zod_1.z.array(zod_1.z.nativeEnum(OntoGenerator_1.AxiomType)).min(1).default([OntoGenerator_1.AxiomType.ONTOLOGICAL, OntoGenerator_1.AxiomType.LOGICAL]),
    customAxioms: zod_1.z.array(zod_1.z.string()).optional().default([]),
    constraints: zod_1.z.array(zod_1.z.string()).optional()
});
const manifestRealitySchema = zod_1.z.object({
    name: zod_1.z.string().min(1).max(100).optional(),
    axiomTypes: zod_1.z.array(zod_1.z.nativeEnum(OntoGenerator_1.AxiomType)).optional().default([OntoGenerator_1.AxiomType.ONTOLOGICAL, OntoGenerator_1.AxiomType.LOGICAL, OntoGenerator_1.AxiomType.CAUSAL]),
    causalityType: zod_1.z.nativeEnum(OntoGenerator_1.CausalityType).optional().default(OntoGenerator_1.CausalityType.EFFICIENT),
    dimensions: zod_1.z.number().int().min(1).max(11).optional().default(4),
    modalSystem: zod_1.z.nativeEnum(OntoGenerator_1.ModalSystem).optional().default(OntoGenerator_1.ModalSystem.S5),
    constraints: zod_1.z.array(zod_1.z.string()).optional().default([])
});
const reweaveCausalitySchema = zod_1.z.object({
    causalWeb: zod_1.z.object({
        id: zod_1.z.string(),
        nodes: zod_1.z.array(zod_1.z.any()),
        topology: zod_1.z.string(),
        temporalConstraints: zod_1.z.array(zod_1.z.string())
    }),
    modifications: zod_1.z.object({
        addLinks: zod_1.z.array(zod_1.z.tuple([zod_1.z.string(), zod_1.z.string()])).optional(),
        removeLinks: zod_1.z.array(zod_1.z.tuple([zod_1.z.string(), zod_1.z.string()])).optional(),
        changeType: zod_1.z.nativeEnum(OntoGenerator_1.CausalityType).optional()
    })
});
const manifestFromENSSchema = zod_1.z.object({
    name: zod_1.z.string().min(1).max(100).optional(),
    potentialTypes: zod_1.z.array(zod_1.z.nativeEnum(PhenomenonWeaver_1.PotentialType)).optional().default([PhenomenonWeaver_1.PotentialType.PURE_BEING, PhenomenonWeaver_1.PotentialType.PURE_LOGIC, PhenomenonWeaver_1.PotentialType.ENS_UNITY]),
    axiomTypes: zod_1.z.array(zod_1.z.nativeEnum(OntoGenerator_1.AxiomType)).optional().default([OntoGenerator_1.AxiomType.ONTOLOGICAL, OntoGenerator_1.AxiomType.LOGICAL]),
    causalityType: zod_1.z.nativeEnum(OntoGenerator_1.CausalityType).optional().default(OntoGenerator_1.CausalityType.EFFICIENT),
    dimensions: zod_1.z.number().int().min(1).max(11).optional().default(4)
});
const observeRealitySchema = zod_1.z.object({
    realityId: zod_1.z.string().uuid(),
    observationType: zod_1.z.nativeEnum(PhenomenonWeaver_1.ObservationType).optional().default(PhenomenonWeaver_1.ObservationType.TRANSCENDENT)
});
const genesisRoutes = async (app) => {
    // ═══════════════════════════════════════════════════════════════════════════════
    ...
    app.addHook('preHandler', async (request, reply) => {
        ...
        if (!publicRoutes.includes(path)) {
            ...
            await (0, auth_1.requireAuth)(request);
        }
    });
    /**
    ...
    app.get('/status', async () => {
        return {
            ...
            createdRealities: OntoGenerator_1.ontoGenerator.getCreatedRealities().length,
            manifestedRealities: PhenomenonWeaver_1.phenomenonWeaver.getAllRealities().length,
    ...
    app.post('/createAxiom', async (request, reply) => {
        try {
            ...
            const system = OntoGenerator_1.ontoGenerator.createAxiomSet(body.name || `AxiomSystem-${Date.now()}`, body.types, body.customAxioms);
            // Track usage
            // Complexity: O(1)
    ...
    app.post('/manifestReality', async (request, reply) => {
        try {
            ...
            if (body.causalityType === OntoGenerator_1.CausalityType.RETROCAUSAL && !limits.allowRetrocausal) {
            ...
            const reality = OntoGenerator_1.ontoGenerator.createReality({
    ...
    app.post('/reweaveCausality', async (request, reply) => {
        try {
            ...
            const rewoven = OntoGenerator_1.ontoGenerator.reweaveCausality(body.causalWeb, body.modifications);
            return {
                success: true,
    ...
    app.get('/availablePotentials', async () => {
        const ontoPotentials = OntoGenerator_1.ontoGenerator.getAvailablePotentials();
        const phenoPotentials = PhenomenonWeaver_1.phenomenonWeaver.getAvailablePotentials();
        return {
            success: true,
    ...
    app.get('/accessENS', async () => {
        const ens = PhenomenonWeaver_1.phenomenonWeaver.accessENS();
        return {
            success: true,
            unity: {
                type: ens.unity.type,
    ...
    app.post('/manifestFromENS', async (request, reply) => {
        try {
            ...
            const axiomSystem = OntoGenerator_1.ontoGenerator.createAxiomSet(`${body.name || 'ENS-Reality'}-Axioms`, body.axiomTypes, []);
            // Create causal structure
            const causalNodes = axiomSystem.axioms.map(a => a.name);
            const causalWeb = OntoGenerator_1.ontoGenerator.causality.createCausalWeb(causalNodes, 'branching');
            // Create spacetime
            const spacetime = OntoGenerator_1.ontoGenerator.hyperArchitect.projectHyperDimension(`${body.name || 'ENS-Reality'}-Spacetime`, body.dimensions, 0, 0.00001);
            // Create modal worlds
            const { worlds } = OntoGenerator_1.ontoGenerator.modalLogic.generateS5System();
            // Manifest from ENS
            const result = PhenomenonWeaver_1.phenomenonWeaver.manifestFromENS({
    ...
    app.post('/observe', async (request, reply) => {
        try {
            ...
            const result = PhenomenonWeaver_1.phenomenonWeaver.observeReality(body.realityId, body.observationType);
            if (!result) {
    ...
    app.post('/stabilize', async (request, reply) => {
        try {
            const { realityId } = zod_1.z.object({ realityId: zod_1.z.string().uuid() }).parse(request.body);
            const stabilized = PhenomenonWeaver_1.phenomenonWeaver.stabilize(realityId);
            if (!stabilized) {
                return reply.status(404).send({
                    success: false,
                    error: 'Reality not found'
    ...
    app.get('/predictEvolution/:realityId', async (request, reply) => {
        ...
        const prediction = PhenomenonWeaver_1.phenomenonWeaver.predictEvolution(realityId, parseInt(steps || '10'));
        return {
            success: true,
            realityId,
    ...
    app.get('/realities', async () => {
        const ontoRealities = OntoGenerator_1.ontoGenerator.getCreatedRealities();
        const phenoRealities = PhenomenonWeaver_1.phenomenonWeaver.getAllRealities();
        return {
            success: true,
    ...
    app.get('/reality/:id', async (request, reply) => {
        ...
        const ontoReality = OntoGenerator_1.ontoGenerator.getReality(id);
        const phenoReality = PhenomenonWeaver_1.phenomenonWeaver.getReality(id);
    ...
    app.delete('/reality/:id', async (request, reply) => {
        const { id } = request.params;
        const result = PhenomenonWeaver_1.phenomenonWeaver.dissolveReality(id);
        if (!result.success) {
            return reply.status(404).send({
                success: false,
    ...
    app.get('/poolStatus', async () => {
        const status = PhenomenonWeaver_1.phenomenonWeaver.manifestationInterface.getPoolStatus();
        return {
            success: true,
        ...
        return {
            ...
            realities: [
                ...
                ...phenoRealities.map(r => ({
                    id: r.id,
                    name: r.name,
                    status: Number(r.coherenceLevel) > 0.5 ? 'STABLE' : 'UNSTABLE',
                    dimensions: 4,
                    containers: 1,
     ...
     */
    app.get('/status/:realityId', async (request, reply) => {
        // SAFETY: async operation — wrap in try-catch for production resilience
        const tenant = await (0, auth_1.getTenant)(request);
        const { realityId } = request.params;
        // Check ownership
        ...
        const ontoReality = OntoGenerator_1.ontoGenerator.getCreatedRealities()
        ...
        const phenoReality = PhenomenonWeaver_1.phenomenonWeaver.getAllRealities()
            .find(r => r.id === realityId);
        if (phenoReality) {
            const coherence = Number(phenoReality.coherenceLevel);
            return {
                success: true,
                id: phenoReality.id,
                name: phenoReality.name,
                status: coherence > 0.5 ? 'STABLE' : 'UNSTABLE',
                coherence: coherence,
                entropy: 1 - coherence,
                dimensions: 4,
                containers: [{
                        ...
                        status: 'running',
                        health: coherence > 0.5 ? 'healthy' : 'starting',
                        ports: ['3000']
                    }],
                observerCount: 0,
     ...
     */
    app.delete('/collapse/:realityId', async (request, reply) => {
        // SAFETY: async operation — wrap in try-catch for production resilience
        const tenant = await (0, auth_1.getTenant)(request);
        const { realityId } = request.params;
        // Check ownership
...
exports.genesisRoutes = genesisRoutes;
