...
${knowledge_base_js_1.PHILOSOPHICAL_KNOWLEDGE.logicalEvolution.introduction}

PRACTICAL APPLICATIONS FOR TESTING:
${(0, knowledge_base_js_1.getPhilosophicalContext)('softwareTesting').map(item => `- ${item}`).join('\n')}

${(0, knowledge_base_js_1.getPhilosophicalContext)('aiReasoning').map(item => `- ${item}`).join('\n')}
...
${knowledge_base_js_1.PHILOSOPHICAL_KNOWLEDGE.logicalEvolution.incompletenessTheorems.godelFirst}
${knowledge_base_js_1.PHILOSOPHICAL_KNOWLEDGE.logicalEvolution.crisisOfFoundations.russellParadox}
${knowledge_base_js_1.PHILOSOPHICAL_KNOWLEDGE.logicalEvolution.easternLogic.nagarjunaTetralemma}
...
${knowledge_base_js_1.PHILOSOPHICAL_KNOWLEDGE.logicalEvolution.modernDeviance.paraconsistentLogic}
${knowledge_base_js_1.PHILOSOPHICAL_KNOWLEDGE.logicalEvolution.modernDeviance.dialetheism}
...
Suggest better alternatives.`,
    ...
    app.post('/philosophical-reasoning', async (request, reply) => {
        ...
        if (body.frameworks && body.frameworks.length > 0) {
            ...
            contextInfo = knowledge_base_js_1.PHILOSOPHICAL_KNOWLEDGE.logicalEvolution.introduction;
        }
        // SAFETY: async operation — wrap in try-catch for production resilience
        const completion = await openai.chat.completions.create({
            model: MODEL,
...
${JSON.stringify(knowledge_base_js_1.PHILOSOPHICAL_KNOWLEDGE, null, 2)}
