...
exports.armedReaper = exports.ArmedReaper = void 0;
const events_1 = require("events");
const ArbitrageOrchestrator_1 = require("./ArbitrageOrchestrator");
const LiveWalletManager_1 = require("./LiveWalletManager");
const EmergencyKillSwitch_1 = require("./EmergencyKillSwitch");
const biometric_jitter_1 = require("../../biology/biometric-jitter");
// ═══════════════════════════════════════════════════════════════════════════
// ARMED REAPER - LIVE EXECUTION ENGINE
// ═══════════════════════════════════════════════════════════════════════════
class ArmedReaper extends events_1.EventEmitter {
    config;
    orchestrator;
    walletManager;
    killSwitch;
    ...
    constructor(config = {}) {
        ...
        this.orchestrator = ArbitrageOrchestrator_1.arbitrageOrchestrator;
        this.walletManager = LiveWalletManager_1.liveWalletManager;
        this.killSwitch = EmergencyKillSwitch_1.emergencyKillSwitch;
        this.biometric = biometric_jitter_1.biometricJitter;
...
╚═══════════════════════════════════════════════════════════════════════════════════════╝
        ...
        if (this.config.enableKillSwitch) {
            this.killSwitch.setWithdrawalAddress(this.config.withdrawalAddress, this.config.withdrawalNetwork);
...
╚═══════════════════════════════════════════════════════════════════════════════════════╝
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // Complexity: O(N)
    async handleTradeOpportunity(opportunity) {
        // Safety checks
        if (!this.canTrade()) {
    ...
    async simulatePaperTrade(record) {
...
╚═══════════════════════════════════════════════════════════════════════════════════════╝
    ...
    canTrade() {
        ...
        if (this.dailyLossLimitReached)
            return false;
        if (this.drawdownLimitReached)
            return false;
        if (this.activeTrades >= this.config.maxConcurrentTrades)
            return false;
        if (!this.biometric.isActiveTime())
            return false;
        return true;
    }
    // Complexity: O(1) — hash/map lookup
...
exports.ArmedReaper = ArmedReaper;
// ═══════════════════════════════════════════════════════════════════════════
// SINGLETON EXPORT
// ═══════════════════════════════════════════════════════════════════════════
exports.armedReaper = new ArmedReaper();
exports.default = ArmedReaper;
