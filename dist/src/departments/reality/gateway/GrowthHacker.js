...
exports.GrowthHacker = void 0;
exports.createGrowthHacker = createGrowthHacker;
 ...
 */
class GrowthHacker extends events_1.EventEmitter {
    config;
    // Data stores
    leads = new Map();
    caseStudies = new Map();
     ...
     */
    // Complexity: O(N) — linear iteration
    async discoverLeadsFromCaseStudy(caseStudyId, limit = 50) {
    ...
    /**
     * Find similar sites by industry (wrapper for Singularity)
     */
    // Complexity: O(N*M) — nested iteration detected
    async findSimilarSitesByIndustry(industries) {
        // this.log('info', `[GROWTH-HACKER] 🔍 Finding similar sites for industries: ${industries.join(', ')}`);
        const allNewLeads = [];
        for (const industry of industries) {
            const mockLeadProfile = this.generateSimulatedLead({
                caseStudyId: 'mock',
            ...
            allNewLeads.push(mockLeadProfile);
        }
        return allNewLeads;
    }
    /**
     ...
     */
    // Complexity: O(N) — linear iteration
    generateSimulatedLead(caseStudy) {
     ...
     */
    // Complexity: O(N) — linear iteration
    selectRandomProtection(protections) {
     ...
     */
    // Complexity: O(1) — hash/map lookup
    async generateColdOffer(leadId) {
     ...
     */
    // Complexity: O(N) — linear iteration
    selectBestTemplate(lead, caseStudy) {
    ...
    buildPersonalizationTokens(lead, caseStudy) {
        const tokens = {
     ...
     */
    // Complexity: O(1) — hash/map lookup
    recordResponse(offerId, positive) {
        const offer = this.offers.get(offerId);
        if (!offer)
            return;
     ...
     */
    // Complexity: O(N) — linear iteration
    recordConversion(leadId, dealValue) {
        const lead = this.leads.get(leadId);
        if (!lead)
            return;
    ...
    async runCampaign(campaignId) {
        ...
        const matchingLeads = Array.from(this.leads.values())
            ...
            if (lead.score < campaign.minScore)
                return false;
            if (campaign.targetIndustry && lead.industry !== campaign.targetIndustry)
                return false;
            return true;
        })
...
exports.GrowthHacker = GrowthHacker;
// ═══════════════════════════════════════════════════════════════════════════
// FACTORY EXPORT
// ═══════════════════════════════════════════════════════════════════════════
/**
 ...
 */
function createGrowthHacker(config) {
    return new GrowthHacker(config);
}
exports.default = GrowthHacker;
