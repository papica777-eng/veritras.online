...
exports.pricingSyncEngine = exports.PricingSyncEngine = void 0;
...
class PricingSyncEngine {
    ...
    async extractArbitrageFeatures() {
        try {
    ...
    async extractGhostProtocolFeatures() {
        try {
    ...
    generatePricingUpdates() {
        ...
        updates.set('ghostProtocol', `Ghost Protocol ${this.manifest.features.ghostProtocol.version}`);
        // Exchange count
        updates.set('exchanges', `${this.manifest.features.arbitrage.exchanges} exchanges`);
        // Symbols count
        updates.set('symbols', `${this.manifest.features.arbitrage.supportedSymbols.length} blockchains`);
        return updates;
    }
    // Complexity: O(1) — hash/map lookup
    async applyPricingUpdates(updates) {
        try {
            ...
            content = content.replace(/<span class="version">v[\d.]+<\/span>/g, `<span class="version">v${this.manifest?.version}<\/span>`);
...
exports.PricingSyncEngine = PricingSyncEngine;
// ═══════════════════════════════════════════════════════════════════════════
// SINGLETON EXPORT
// ═══════════════════════════════════════════════════════════════════════════
exports.pricingSyncEngine = new PricingSyncEngine();
exports.default = PricingSyncEngine;
// ═══════════════════════════════════════════════════════════════════════════
// CLI INTEGRATION
// ═══════════════════════════════════════════════════════════════════════════
