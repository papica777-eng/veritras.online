...
class ProfitOptimizer extends events_1.EventEmitter {
    ...
    registerWorker(workerId, provider, region, instanceType, pricingModel = 'spot') {
        ...
        const hourlyRate = pricingModel === 'spot' ? instancePricing.spotPrice : instancePricing.onDemandPrice;
    ...
    generateRecommendedActions() {
        ...
        const nonSpotWorkers = Array.from(this.workers.values()).filter((w) => w.pricingModel !== 'spot' && w.status === 'active');
        if (nonSpotWorkers.length > 0) {
            actions.push(`Convert ${nonSpotWorkers.length} workers to spot instances`);
        }
        // Check expensive regions
        const expensiveWorkers = Array.from(this.workers.values()).filter((w) => w.marginPercent < 50 && w.status === 'active');
