...
class SandboxExecutor extends node_events_1.EventEmitter {
    ...
    async validateMutation(mutationId, mutationCode, testContext = {}) {
        ...
        const isMalicious = sandboxResult.violations.some((v) => v.type === 'process' || v.type === 'filesystem' || v.type === 'network');
        // Determine if mutation is unstable
        const isUnstable = sandboxResult.violations.some((v) => v.type === 'timeout' || v.type === 'memory') ||
            !!sandboxResult.error;
