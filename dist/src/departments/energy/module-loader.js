...
exports.HotSwapModuleLoader = void 0;
 ...
 */
class HotSwapModuleLoader extends events_1.EventEmitter {
    /** Configuration */
    config;
    /** Registered modules */
    modules = new Map();
    ...
    registerModule(name, instance, description) {
        ...
        const methodNames = Object.getOwnPropertyNames(Object.getPrototypeOf(instance)).filter((name) => name !== 'constructor' && typeof instance[name] === 'function');
        for (const methodName of methodNames) {
            const method = this.registerMethod(moduleId, methodName, instance[methodName].bind(instance));
    ...
    registerMethod(moduleId, methodName, implementation) {
     ...
     */
    // Complexity: O(1)
    addAlternative(methodId, implementation, options) {
     ...
     */
    // Complexity: O(N) — linear iteration
    async swap(methodId, alternativeId) {
        const method = this.methods.get(methodId);
        if (!method) {
            throw new Error(`Method not found: ${methodId}`);
        }
     ...
     */
    // Complexity: O(1)
    updateAverageTime(method, executionTime) {
     ...
     */
    // Complexity: O(1)
    updateAlternativeTime(alternative, executionTime) {
        const alpha = 0.2;
    ...
    // Complexity: O(N) — linear iteration
    getStats() {
        const activeSwaps = Array.from(this.methods.values()).filter((m) => m.activeAlternativeId !== undefined).length;
        return {
            ...this.stats,
...
exports.HotSwapModuleLoader = HotSwapModuleLoader;
exports.default = HotSwapModuleLoader;
