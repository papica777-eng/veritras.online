...
class HardwareLock extends events_1.EventEmitter {
    ...
    async getCpuId() {
        try {
            if (os.platform() === 'win32') {
                const result = (0, child_process_1.execSync)('wmic cpu get processorid', {
                    encoding: 'utf8',
                    windowsHide: true,
                });
                ...
                const result = (0, child_process_1.execSync)("cat /proc/cpuinfo | grep 'Serial\\|model name' | head -2", {
                    encoding: 'utf8',
                });
    ...
    async getMotherboardSerial() {
        try {
            if (os.platform() === 'win32') {
                const result = (0, child_process_1.execSync)('wmic baseboard get serialnumber', {
                    encoding: 'utf8',
                    windowsHide: true,
                });
                const lines = result.trim().split('\n');
    ...
    async getBiosSerial() {
        try {
            if (os.platform() === 'win32') {
                const result = (0, child_process_1.execSync)('wmic bios get serialnumber', {
                    encoding: 'utf8',
                    windowsHide: true,
                });
                const lines = result.trim().split('\n');
                ...
                const result = (0, child_process_1.execSync)('sudo dmidecode -s bios-version 2>/dev/null || echo "unknown"', {
                    encoding: 'utf8',
                });
                return result.trim();
            }
    ...
    async getDiskUUID() {
        try {
            if (os.platform() === 'win32') {
                const result = (0, child_process_1.execSync)('wmic diskdrive get serialnumber', {
                    encoding: 'utf8',
                    windowsHide: true,
                });
                const lines = result.trim().split('\n');
                ...
                const result = (0, child_process_1.execSync)('blkid -o value -s UUID /dev/sda1 2>/dev/null || echo "unknown"', {
                    encoding: 'utf8',
                });
                return result.trim();
            }
            else if (os.platform() === 'darwin') {
                const result = (0, child_process_1.execSync)('diskutil info disk0 | grep "Disk / Partition UUID"', {
                    encoding: 'utf8',
                });
    ...
    async getGpuInfo() {
        try {
            if (os.platform() === 'win32') {
                const result = (0, child_process_1.execSync)('wmic path win32_VideoController get name', {
                    encoding: 'utf8',
                    windowsHide: true,
                });
