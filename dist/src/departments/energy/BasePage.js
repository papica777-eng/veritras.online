...
class BasePage {
    ...
    async getOptimizedDOMSnapshot(targetSelector) {
        // SAFETY: async operation — wrap in try-catch for production resilience
        const { optimizeDOM } = await Promise.resolve().then(() => __importStar(require('../apps/worker/src/utils/dom-optimizer')));
        // SAFETY: async operation — wrap in try-catch for production resilience
    ...
    async requestAIHealing(brokenSelector, error) {
        // SAFETY: async operation — wrap in try-catch for production resilience
        const { optimizeDOM, suggestSelectors } = await Promise.resolve().then(() => __importStar(require('../apps/worker/src/utils/dom-optimizer')));
