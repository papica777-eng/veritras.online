...
exports.PERSPECTIVES = exports.createMultiPerspectiveAnalyzer = exports.MultiPerspectiveAnalyzer = void 0;
// ═══════════════════════════════════════════════════════════════════════════════
...
class MultiPerspectiveAnalyzer {
    ...
    analyze(problem, selectedPerspectives) {
        ...
        const analyses = perspectivesToUse.map(p => p.analyzeFunc(problem));
        // Синтез на резултатите
        const synthesis = this.synthesize(analyses);
        // Намиране на доминираща перспектива
        const dominant = analyses.reduce((best, curr) => curr.confidence > best.confidence ? curr : best);
        return {
            problem,
            analyses,
            synthesis,
    ...
    findConflicts(analyses) {
        ...
        for (let i = 0; i < analyses.length; i++) {
            for (let j = i + 1; j < analyses.length; j++) {
                ...
                const conflictingActions = this.findContradictions(a1.suggestedActions, a2.suggestedActions);
    ...
    // Complexity: O(N*M) — nested iteration detected
    findContradictions(actions1, actions2) {
        ...
        for (const a1 of actions1) {
            for (const a2 of actions2) {
                for (const [pos, neg] of opposites) {
                    if ((a1.toLowerCase().includes(pos) && a2.toLowerCase().includes(neg)) ||
    ...
    // Complexity: O(1)
    suggestResolution(a1, a2, conflict) {
    ...
    assessImportance(conflict) {
        ...
        if (highPriority.some(w => lower.includes(w)))
            return 'high';
        if (lowPriority.some(w => lower.includes(w)))
            return 'low';
        return 'medium';
    }
    // Complexity: O(N log N) — sort operation
    generateRecommendation(analyses, commonInsights, conflicts) {
        const lines = [];
...
exports.MultiPerspectiveAnalyzer = MultiPerspectiveAnalyzer;
// ═══════════════════════════════════════════════════════════════════════════════
// EXPORTS
// ═══════════════════════════════════════════════════════════════════════════════
const createMultiPerspectiveAnalyzer = () => {
    return new MultiPerspectiveAnalyzer();
};
exports.createMultiPerspectiveAnalyzer = createMultiPerspectiveAnalyzer;
exports.default = MultiPerspectiveAnalyzer;
