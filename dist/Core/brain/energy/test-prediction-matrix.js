...
async function runTests() {
    ...
    try {
        const prediction = await index_1.predictionMatrix.predictBestSelector(mockElement, mockContext);
    ...
    try {
        index_1.predictionMatrix.reportOutcome(mockElement, mockElement.selectors[0], mockContext, mockOutcome);
    ...
    try {
        const stats = index_1.predictionMatrix.getStatistics();
        console.log('   ✅ Statistics retrieved!');
        console.log(`   🧬 Evolution Tracker: ${stats.evolutionTracker.trackedElements} elements, ${stats.evolutionTracker.totalMutations} mutations`);
        console.log(`   🔮 Simulator: ${stats.simulator.cachedSimulations} cached, ${stats.simulator.scenarioCount} scenarios`);
        console.log(`   🤖 RL Bridge: Q-table size ${stats.rlBridge.qTableSize}, exploration rate ${(stats.rlBridge.explorationRate * 100).toFixed(1)}%`);
    }
    catch (error) {
    ...
    console.log('╚═══════════════════════════════════════════════════════════════╝');
    // Cleanup
    index_1.predictionMatrix.dispose();
}
// Run tests
// Complexity: O(1)
