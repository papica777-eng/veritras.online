...
class GhostProtocol extends events_1.EventEmitter {
     ...
     */
    generateScreenResolution(entropy) {
        const resolutions = [
     ...
     */
    generateHardwareConcurrency(entropy) {
     ...
     */
    obfuscateRequest(request, config) {
        ...
        if (config.headers) {
            obfuscated.headers = this.obfuscateHeaders(request.headers || {}, entropy.slice(2, 6));
        }
     ...
     */
    obfuscateHeaders(headers, entropy) {
     ...
     */
    recordDetectionAttempt(detected) {
