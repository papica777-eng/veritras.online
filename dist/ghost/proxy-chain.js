...
// 🔄 PROXY CHAIN ENGINE
// ═══════════════════════════════════════════════════════════════════════════════════════
class ProxyChain extends events_1.EventEmitter {
    config;
    state;
    healthReports;
    rotationTimer;
    MAX_FAILURES = 3;
    DEFAULT_ROTATION_INTERVAL = 300000; // 5 minutes
    constructor() {
        super();
    ...
    buildChain() {
        ...
        // Calculate metrics
        const estimatedLatency = selectedNodes.reduce((sum, node) => sum + (node.latency || 100), 0);
     ...
     */
    selectOptimalNode(nodes, usedCountries) {
        if (nodes.length === 0)
            return null;
        // Score each node
     ...
     */
    reportResult(nodeId, success, latency) {
     ...
     */
    updateHealthReport(nodeId, success, latency) {
