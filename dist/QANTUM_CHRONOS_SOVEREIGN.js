...
exports.OBIManifold = exports.SoulExecutor = exports.CounterfactualEngine = exports.RetrocausalEngine = exports.ChronosSovereign = void 0;
...
exports.SoulExecutor = SoulExecutor;
class OBIManifold {
    dimensions;
    curvatureThreshold;
    constructor(dims = 1024, threshold = 0.85) {
        this.dimensions = dims;
        this.curvatureThreshold = threshold;
    }
    // Complexity: O(n) where n = number of order book levels
    calculateManifoldCurvature(bids, asks) {
        let totalBidVol = 0;
        let totalAskVol = 0;
        ...
        let topology = 'STABLE';
        if (curvature > this.curvatureThreshold * 2)
            topology = 'SINGULARITY';
        else if (curvature > this.curvatureThreshold)
            topology = 'COLLAPSING';
...
exports.OBIManifold = OBIManifold;
class CounterfactualEngine {
    ...
    evaluateCounterfactual(decision, currentPrice, priceHistory) {
        const alternatives = ['BUY', 'SELL', 'HOLD'];
        ...
        const projectedOutcomes = horizons.map(h => {
            ...
            if (decision.action === 'BUY')
                originalPnL = priceDelta;
            else if (decision.action === 'SELL')
                originalPnL = -priceDelta;
            // Изчисляваме PnL за алтернативното решение
            if (alternativeAction === 'BUY')
                alternativePnL = priceDelta;
            else if (alternativeAction === 'SELL')
                alternativePnL = -priceDelta;
            const regretDelta = originalPnL - alternativePnL;
            // Catuskoti verdict
            let catuskotiVerdict;
            if (regretDelta > 1)
                catuskotiVerdict = 'TRUE'; // Original was clearly right
            else if (regretDelta < -1)
                catuskotiVerdict = 'FALSE'; // Original was clearly wrong
            else if (Math.abs(regretDelta) <= 1 && futurePrice)
                catuskotiVerdict = 'PARADOX'; // Too close to call
            else
                catuskotiVerdict = 'TRANSCENDENT'; // Beyond measurement
            return {
                horizon: h.name,
                originalPnL,
                alternativePnL,
    ...
    findPriceAtHorizon(history, decisionTime, horizonMs) {
        ...
        if (closest && minDiff < horizonMs * 0.1)
...
class RetrocausalEngine {
    ...
    analyzeTemporalRegret(decisionHistory, currentPrice, priceHistory) {
        const analyses = [];
        ...
        for (const decision of decisionHistory) {
            ...
            const soulResult = this.soul.execute(`${decision.action}:${decision.pair}:${decision.price}:${decision.confidence}`);
            ...
            if (shortRegret < 0 && longRegret > 0) {
                ...
                this.createRetrocausalLink(`Future-profit-${decision.pair}`, `Past-loss-${decision.id}`, 0.7, `Решение "${decision.action}" изглежда грешно на 1h (-${Math.abs(shortRegret).toFixed(1)}%), ` +
                ...
                this.createRetrocausalLink(`Future-loss-${decision.pair}`, `Past-profit-${decision.id}`, 0.5, `Решение "${decision.action}" изглежда правилно на 1h (+${shortRegret.toFixed(1)}%), ` +
    ...
    createRetrocausalLink(futureEvent, pastDecision, strength, verdict) {
        this.links.push({
            futureEvent,
            pastDecision,
    ...
    getSoulState() { return this.soul.getState(); }
}
exports.RetrocausalEngine = RetrocausalEngine;
class ChronosSovereign {
    ...
    ingestDecision(pair, action, price, confidence, catuskotiState, bids = [], asks = []) {
        const manifoldSignal = bids.length > 0
            ? this.manifold.calculateManifoldCurvature(bids, asks)
            : { curvature: 0, imbalance: 0, omegaActive: false, topology: 'STABLE' };
    ...
    generateSovereignReport(currentPrice, priceHistory) {
        const analyses = this.retro.analyzeTemporalRegret(this.decisionLog, currentPrice, priceHistory);
        ...
        return {
            ...
            soulState,
            manifoldTopology: latestManifold.topology,
...
exports.ChronosSovereign = ChronosSovereign;
// ═══════════════════════════════════════════════════════════════════════════════
...
async function main() {
    ...
    const d4 = chronos.ingestDecision('BTCEUR', 'BUY', 62590, 0.92, 'TRANSCENDENT', [100, 200, 150, 300, 250, 180], // Bids
