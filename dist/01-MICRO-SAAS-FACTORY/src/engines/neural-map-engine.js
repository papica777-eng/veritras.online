...
class NeuralMapEngine {
    ...
    async createAnchor(page, element, name, businessFunction) {
        ...
        logger_1.logger.debug(`🧠 [NEURAL] Creating Cognitive Anchor: ${name}`);
        ...
        this.saveAnchors();
        logger_1.logger.debug(`🧠 [NEURAL] Anchor created with ${selectors.length} selector signals`);
        logger_1.logger.debug(`   Top selector: ${selectors[0]?.selector} (${selectors[0]?.confidence}% confidence)`);
        return anchor;
    }
    /**
    ...
    async findElement(page, anchorId) {
        ...
        if (!anchor) {
            logger_1.logger.warn(`🧠 [NEURAL] Anchor not found: ${anchorId}`);
            return null;
        }
        this.currentPage = page;
            ...
            try {
                ...
                if (element) {
                    ...
                    if (this.config.visualFingerprintEnabled) {
                        ...
                        if (!isMatch) {
                            logger_1.logger.debug(`🧠 [NEURAL] Visual mismatch for ${signal.type}, trying next`);
                            continue;
                        }
                    }
        ...
        if (this.config.autoHealingEnabled) {
            logger_1.logger.debug(`🧠 [NEURAL] All selectors failed, initiating self-healing...`);
    ...
    async buildDomAst(page) {
        logger_1.logger.debug(`🧠 [NEURAL] Building DOM AST...`);
        ...
        this.pageAst = ast;
        logger_1.logger.debug(`🧠 [NEURAL] AST built with ${this.countNodes(ast)} nodes`);
        return ast;
    }
    // ============================================================
    ...
    async selfHeal(page, anchor) {
        logger_1.logger.debug(`🩹 [NEURAL] Self-healing anchor: ${anchor.name}`);
        ...
        logger_1.logger.error(`🩹 [NEURAL] Self-healing failed for: ${anchor.name}`);
    ...
    async updateAnchorFromElement(page, anchor, element, healingMethod) {
        logger_1.logger.debug(`🩹 [NEURAL] Healed using: ${healingMethod}`);
    ...
    loadAnchors() {
        ...
        try {
            if (fs.existsSync(filePath)) {
                ...
                logger_1.logger.debug(`🧠 [NEURAL] Loaded ${this.anchors.size} anchors`);
            }
        }
        catch (error) {
            logger_1.logger.warn('🧠 [NEURAL] Could not load anchors');
        }
    }
    saveAnchors() {
        const filePath = path.join(this.config.storageDir, 'anchors.json');
        try {
            ...
            logger_1.logger.error('🧠 [NEURAL] Could not save anchors');
        }
    }
}
