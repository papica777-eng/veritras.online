...
exports.CommonIntents = exports.AdaptiveSemanticCore = void 0;
...
class AdaptiveSemanticCore {
    ...
    async createSemanticMap(page) {
        ...
        const elements = await page.evaluate(() => {
            ...
            const getNearbyText = (el) => {
                ...
                if (id) {
                    ...
                    if (label)
                        nearby.push(label.textContent?.trim() || '');
                }
        ...
        const semanticElements = elements.map((el, idx) => ({
            ...el,
     ...
     */
    expandKeywords(keywords, customSynonyms) {
     ...
     */
    calculateScore(element, intent, keywords) {
        let score = 0;
    ...
    textMatchScore(element, keywords) {
    ...
    typeMatchScore(element, expectedType) {
        ...
        if (expected.includes(element.tag))
            return 1;
        if (element.role === expectedType)
            return 0.9;
        if (element.classes.some(c => c.includes(expectedType)))
            return 0.7;
        return 0;
    }
    positionMatchScore(element, hint) {
        if (!hint)
            return 0.5;
        ...
        switch (hint) {
            ...
            case 'center':
                const centerDist = Math.sqrt(Math.pow(centerX - viewportWidth / 2, 2) +
    ...
    ariaMatchScore(element, keywords) {
    ...
    contextMatchScore(element, keywords) {
    ...
    getMatchReasons(element, intent, keywords) {
        const reasons = [];
        const keywordArray = Array.from(keywords);
        const text = element.visibleText.toLowerCase();
        keywordArray.forEach(k => {
            if (text.includes(k))
                reasons.push(`Text contains "${k}"`);
        });
     ...
     */
    async visualFallback(page, intent, description) {
        ...
        // Take screenshot
        const screenshotPath = path.join(path.dirname(this.config.knowledgePath), `screenshot_${Date.now()}.png`);
     ...
     */
    async extractSelectorFromCoordinates(page, x, y) {
    ...
    saveKnowledge() {
        try {
            ...
            fs.writeFileSync(this.config.knowledgePath, JSON.stringify(data, null, 2), 'utf-8');
            if (this.config.verbose) {
     ...
     */
    learnSelector(intent, url, element, success) {
    ...
    async executeIntent(page, intent, action = 'click', value) {
        ...
        if (!match || match.score < this.config.confidenceThreshold) {
            if (this.config.enableVisualFallback) {
                const visualMatch = await this.visualFallback(page, intent, intent.keywords.join(' '));
...
exports.AdaptiveSemanticCore = AdaptiveSemanticCore;
// ═══════════════════════════════════════════════════════════════════════════
// EXPORTS
// ═══════════════════════════════════════════════════════════════════════════
exports.default = AdaptiveSemanticCore;
// Pre-defined common intents
exports.CommonIntents = {
    LOGIN: {
        action: 'LOGIN_ACTION',
