"use strict";
/**
 * SandboxGuard — Qantum Module
 * @module SandboxGuard
 * @path omni_core/security/SandboxGuard.ts
...
class SandboxGuard {
    ...
    constructor() {
        ...
        if (!this.isLinux) {
            this.logger.warn('SANDBOX', '⚠️ eBPF sandbox unavailable on non-Linux platform. Using isolated-vm fallback (reduced kernel-level enforcement).');
        }
    }
    static getInstance() {
     ...
     */
    // Complexity: O(1)
    async executeSecurely(code, timeout = 5000, allowedModules = []) {
    ...
    async executeWithLinuxNamespaces(code, timeout, allowedModules) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const sandboxProcess = (0, child_process_1.spawn)('unshare', [
                '--map-root-user',
                '--net',
                '--pid',
                '--fork',
    ...
    async executeWithIsolatedVM(code, timeout, allowedModules) {
        this.logger.debug('SANDBOX', '🔐 Using isolated-vm (V8 isolate-based sandbox)');
        const isolate = new isolated_vm_1.default.Isolate({ memoryLimit: 128 }); // 128MB memory limit
        ...
        try {
            ...
            if (allowedModules.length > 0) {
                ...
                for (const mod of allowedModules) {
                    const moduleRef = new isolated_vm_1.default.Reference(require(mod));
     ...
     */
    // Complexity: O(1)
    wrapCodeWithSecurityContext(code, allowedModules) {
...
exports.SandboxGuard = SandboxGuard;
