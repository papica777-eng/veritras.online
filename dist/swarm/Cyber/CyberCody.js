...
exports.CyberCody = void 0;
exports.getCyberCody = getCyberCody;
exports.resetCyberCody = resetCyberCody;
...
class CyberCody {
    ...
    constructor(config = {}) {
        ...
        this.sharedMemory = (0, SharedMemoryV2_1.getSharedMemory)('cyber_cody');
        this.stats = {
            totalAudits: 0,
            passedAudits: 0,
            failedAudits: 0,
     ...
     */
    async auditSignal(signal, assetId) {
     ...
     */
    validateLeverage(signal, issues, recommendations) {
     ...
     */
    validatePositionSize(signal, issues, recommendations) {
     ...
     */
    validateRiskReward(signal, issues, recommendations) {
     ...
     */
    validatePriceLogic(signal, issues, _recommendations) {
     ...
     */
    validateMarketConditions(signal, issues, recommendations) {
     ...
     */
    calculateRiskScore(signal, issues) {
     ...
     */
    calculateConfidence(signal, issues) {
     ...
     */
    determinePassFail(confidence, riskScore, issues) {
        // Automatic fail for critical logic holes
        const hasCriticalIssue = issues.some(i => i.type === 'LOGIC_HOLE' && i.severity >= 1.0);
        if (hasCriticalIssue) {
            return false;
        }
     ...
     */
    async triggerSelfHealing(assetId, auditResult) {
        ...
        try {
            const spawner = (0, AssetSpawner_1.getAssetSpawner)();
     ...
     */
    async syncToSharedMemory(lastResult) {
 ...
 */
let globalCody = null;
function getCyberCody(config) {
    if (!globalCody) {
        globalCody = new CyberCody(config);
    }
    return globalCody;
}
function resetCyberCody() {
    globalCody = null;
}
