...
exports.getAssetSpawner = getAssetSpawner;
exports.resetAssetSpawner = resetAssetSpawner;
...
class AssetSpawner {
    ...
    constructor(config = {}) {
        ...
        this.sharedMemory = (0, SharedMemoryV2_1.getSharedMemory)('asset_spawner');
    ...
    analyzeMarketVoid(marketVoid) {
        ...
        const score = (marketVoid.demandScore * 0.4 +
    ...
    async spawnAsset(marketVoid) {
        ...
        const asset = {
            config,
            state: types_1.AssetLifecycleState.SPAWNING,
        ...
        try {
            ...
            asset.state = types_1.AssetLifecycleState.ACTIVE;
            ...
            asset.state = types_1.AssetLifecycleState.ERROR;
     ...
     */
    async deployAsset(asset, template) {
        asset.state = types_1.AssetLifecycleState.DEPLOYING;
     ...
     */
    simulateDeploymentStep(delayMs) {
        return new Promise(resolve => setTimeout(resolve, delayMs));
    }
    /**
     * Determine SaaS type based on required features
     */
    determineSaaSType(features) {
        const keywords = {
        ...
        for (const [type, typeKeywords] of Object.entries(keywords)) {
            const score = features.filter(f => typeKeywords.some(k => f.toLowerCase().includes(k))).length;
            if (score > bestScore) {
                bestScore = score;
                bestMatch = type;
            }
     ...
     */
    calculateOptimalPrice(marketVoid) {
    ...
    async terminateAsset(assetId) {
        ...
        asset.state = types_1.AssetLifecycleState.TERMINATING;
        try {
            ...
            await this.simulateDeploymentStep(100);
            asset.state = types_1.AssetLifecycleState.TERMINATED;
     ...
     */
    recordRevenue(assetId, amount, type, provider, providerTransactionId) {
        const asset = this.assets.get(assetId);
        if (!asset) {
            return false;
        }
    ...
    getActiveAssets() {
        return Array.from(this.assets.values())
            .filter(a => a.state === types_1.AssetLifecycleState.ACTIVE);
    }
    /**
     * Get asset by ID
     */
    ...
    getStats() {
        ...
        const activeAssets = assets.filter(a => a.state === types_1.AssetLifecycleState.ACTIVE);
        return {
 ...
 * Singleton factory
 */
let globalSpawner = null;
function getAssetSpawner(config) {
    if (!globalSpawner) {
        globalSpawner = new AssetSpawner(config);
    }
    return globalSpawner;
}
function resetAssetSpawner() {
    if (globalSpawner) {
