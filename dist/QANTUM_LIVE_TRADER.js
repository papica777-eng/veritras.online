...
const live_connector_1 = require("./system/live-connector");
// ═══════════════════════════════════════════════════════════════════════════════
// SAFETY CONSTANTS
// ═══════════════════════════════════════════════════════════════════════════════
...
// Complexity: O(n)
function getSymbolAsset(symbol) {
...
async function updatePrices() {
    const allPrices = await (0, live_connector_1.fetchAllBinancePrices)();
    ...
    for (const sym of TRADE_SYMBOLS) {
        ...
        history.push(price);
        if (history.length > PRICE_WINDOW)
            history.shift();
...
// ═══════════════════════════════════════════════════════════════════════════════
// Complexity: O(1)
async function executeBuy(symbol, eurAmount, price, action, catuskotiState, confidence) {
    ...
    const result = await (0, live_connector_1.placeMarketOrder)(symbol, 'BUY', qty);
...
// Complexity: O(1)
async function executeSell(symbol, position, price, reason, catuskotiState, confidence) {
    log(`  >> SELL ${symbol}: qty=${position.qty} @ ${price.toFixed(2)} (${reason})`);
    const result = await (0, live_connector_1.placeMarketOrder)(symbol, 'SELL', position.qty);
    if (result.status === 'FILLED') {
        const fillPrice = result.cummulativeQuoteQty / result.executedQty;
...
async function runDecisionCycle(brain, currentEurBalance) {
    totalCycles++;
    ...
    for (const symbol of TRADE_SYMBOLS) {
        ...
        if (!price || !history || history.length < 4)
            continue;
...
async function checkPortfolioSafety() {
    ...
    for (const [symbol, position] of openPositions) {
        const price = await (0, live_connector_1.getPrice)(symbol);
        if (price) {
...
async function main() {
    ...
    console.log('');
    // Validate API keys
    const keyStatus = (0, live_connector_1.validateKeys)();
    ...
    while (isRunning) {
        try {
            ...
            // Get latest balance
            const freshBalances = await (0, live_connector_1.getAccountBalances)();
