...
exports.SupremeMeditation = void 0;
const assimilator_1 = require("./assimilator");
const DependencyGraph_1 = require("../src/cognition/DependencyGraph");
...
// ═══════════════════════════════════════════════════════════════════════════════
class SupremeMeditation {
    projectRoot;
    assimilator;
    dependencyGraph;
    outputDir;
    // State
    assimilationResult = null;
    graphAnalysis = null;
    constructor(projectRoot) {
        this.projectRoot = projectRoot;
        this.outputDir = (0, path_1.join)(projectRoot, 'data', 'supreme-meditation');
        this.assimilator = assimilator_1.Assimilator.getInstance({ targetFolder: (0, path_1.join)(projectRoot, 'src') });
        this.dependencyGraph = DependencyGraph_1.DependencyGraph.getInstance(projectRoot);
    }
    /**
    ...
    async meditate() {
        ...
        const overallHealth = this.calculateOverallHealth(assimilationPhase, layerAuditPhase, deadSymbolsPhase, contextTestPhase);
        // Generate recommendations
        const recommendations = this.generateRecommendations(layerAuditPhase, deadSymbolsPhase, contextTestPhase);
    ...
    async phase2_CrossLayerAudit() {
        ...
        if (violations.length > 0) {
            ...
            for (const v of violations.slice(0, 5)) {
                console.log(`      ❌ ${(0, path_1.basename)(v.source)} (${v.sourceLayer}) → ${(0, path_1.basename)(v.target)} (${v.targetLayer})`);
            }
        ...
        if (circular.length > 0) {
            ...
            for (const c of circular.slice(0, 3)) {
                const shortCycle = c.cycle.map(f => (0, path_1.basename)(f)).join(' → ');
    ...
    // PHASE 3: DEAD SYMBOL DETECTION
    // ─────────────────────────────────────────────────────────────────────────
    async phase3_DeadSymbolDetection() {
        ...
        for (const [name, filePath] of registry.exports) {
            ...
            for (const file of this.assimilationResult.files) {
                ...
                const hasImport = file.imports.some(imp => imp.imports.includes(name));
                if (hasImport) {
                    isUsed = true;
                    break;
                }
    ...
    // PHASE 4: CONTEXT INJECTION TEST
    // ─────────────────────────────────────────────────────────────────────────
    async phase4_ContextInjectionTest(moduleName) {
        console.log(`   📖 Testing context injection for module: ${moduleName}`);
        const mismatches = [];
        const modulePath = (0, path_1.join)(this.projectRoot, 'src', moduleName);
        const docsPath = (0, path_1.join)(this.projectRoot, 'docs');
    ...
    extractSymbolsFromContext(context) {
    ...
    async extractDocumentedSymbols(docsPath, moduleName) {
        const symbols = new Set();
        ...
        try {
            ...
            for (const file of files) {
                if (!file.endsWith('.md'))
                    continue;
                const content = await (0, promises_1.readFile)((0, path_1.join)(docsPath, file), 'utf-8');
    ...
    calculateOverallHealth(assimilation, layerAudit, deadSymbols, contextTest) {
        let health = 100;
    ...
    generateRecommendations(layerAudit, deadSymbols, contextTest) {
        const recommendations = [];
        // Layer violations
        if (layerAudit.violations.length > 0) {
            recommendations.push(`🏗️ Fix ${layerAudit.violations.length} layer violations to maintain clean architecture. ` +
        ...
        if (layerAudit.circularDependencies.length > 0) {
            recommendations.push(`🔄 Break ${layerAudit.circularDependencies.length} circular dependencies. ` +
                `Consider extracting shared code into a separate utility module.`);
        }
        // Dead symbols
        if (deadSymbols.totalDead > 10) {
            recommendations.push(`💀 Consider 'The Great Purge v2': Remove ${deadSymbols.totalDead} unused symbols ` +
                `to reduce codebase complexity.`);
        }
        // Documentation
        if (contextTest.coverage < 80) {
            recommendations.push(`📝 Documentation coverage is ${contextTest.coverage.toFixed(1)}%. ` +
        ...
        if (hallucinations.length > 0) {
            recommendations.push(`👻 Found ${hallucinations.length} documented symbols that don't exist in code. ` +
    ...
    async saveResults(result) {
        // Save JSON
        await (0, promises_1.writeFile)((0, path_1.join)(this.outputDir, 'meditation-result.json'), JSON.stringify(result, null, 2));
        // Save Markdown report
        const report = this.generateMarkdownReport(result);
        await (0, promises_1.writeFile)((0, path_1.join)(this.outputDir, 'meditation-report.md'), report);
        // Save dependency graph
        await this.dependencyGraph.saveAll((0, path_1.join)(this.outputDir, 'dependency-graph'));
    ...
    generateMarkdownReport(result) {
        ...
        if (result.layerAudit.circularDependencies.length > 0) {
            ...
            for (const c of result.layerAudit.circularDependencies.slice(0, 10)) {
                lines.push(`- 🔄 ${c.cycle.map(f => `\`${(0, path_1.basename)(f)}\``).join(' → ')}`);
...
exports.SupremeMeditation = SupremeMeditation;
// ═══════════════════════════════════════════════════════════════════════════════
// CLI EXECUTION
// ═══════════════════════════════════════════════════════════════════════════════
...
exports.default = SupremeMeditation;
