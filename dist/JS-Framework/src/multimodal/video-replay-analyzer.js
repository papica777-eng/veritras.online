...
exports.VideoReplayAnalyzer = void 0;
const events_1 = require("events");
// ═══════════════════════════════════════════════════════════════════════════════
// 📹 VIDEO REPLAY ANALYZER CLASS
// ═══════════════════════════════════════════════════════════════════════════════
class VideoReplayAnalyzer extends events_1.EventEmitter {
    config = null;
    analysisHistory = [];
    frameCache = new Map();
    constructor() {
     ...
     */
    async analyzeVideo(videoBuffer, filename = 'session.mp4') {
        if (!this.config) {
     ...
     * Extract video metadata
     */
    async extractVideoMetadata(videoBuffer) {
     ...
     */
    async extractFrames(videoBuffer, frameRate) {
     ...
     */
    async analyzeFrame(frameData, frameIndex, fps) {
        if (!this.config) {
...
Return structured JSON with these fields:
{
  ...
  "pageContext": { "url": string, "title": string }
     ...
     * Call Gemini Vision API
     */
    async callVisionAPI(imageData, prompt) {
        if (!this.config) {
            throw new Error('Not configured');
        }
     ...
     */
    parseVisionResponse(response, frameIndex, timestamp) {
        ...
        const elements = this.parseElements(parsed.elements || []);
        return {
            frameIndex,
            timestamp,
            elements,
     ...
     */
    parseElements(rawElements) {
        return rawElements.map((raw) => {
            const elem = raw;
            return {
                type: elem.type || 'unknown',
                boundingBox: elem.boundingBox || { x: 0, y: 0, width: 0, height: 0 },
     ...
     */
    detectFrameChanges(prev, current) {
        const changes = [];
     ...
     */
    extractActionSequences(frameAnalyses) {
        const actions = [];
        for (let i = 0; i < frameAnalyses.length; i++) {
            ...
            if (frame.detectedAction && frame.detectedAction.confidence >= (this.config?.confidenceThreshold || 0.7)) {
                ...
                if (frame.cursorPosition) {
                    target = this.findElementAtPosition(frame.elements, frame.cursorPosition.x, frame.cursorPosition.y);
                }
                if (!target && frame.elements.length > 0) {
                    // Use most likely target
                    target = frame.elements.reduce((best, elem) => elem.confidence > best.confidence ? elem : best);
                }
                if (target) {
                    actions.push({
     ...
     */
    findElementAtPosition(elements, x, y) {
     ...
     * Merge consecutive similar actions
     */
    mergeConsecutiveActions(actions) {
        if (actions.length < 2)
            return actions;
        const merged = [];
     ...
     */
    async extractSovereignGoals(actions, frameAnalyses, videoInfo) {
        const goals = [];
     ...
     * Group actions into logical sequences
     */
    groupActionsIntoSequences(actions) {
        const sequences = [];
        let currentSequence = [];
     ...
     */
    async createSovereignGoal(actions, frameAnalyses, videoInfo) {
        if (actions.length === 0)
            return null;
     ...
     */
    generateGoalDescription(actions, goalType) {
        const descriptions = {
            authentication: (acts) => {
     ...
     */
    determineGoalPriority(goalType, actions) {
        const priorities = {
     ...
     */
    calculateGoalConfidence(actions) {
        if (actions.length === 0)
            return 0;
...
exports.VideoReplayAnalyzer = VideoReplayAnalyzer;
// ═══════════════════════════════════════════════════════════════════════════════
// 📦 EXPORTS
// ═══════════════════════════════════════════════════════════════════════════════
exports.default = VideoReplayAnalyzer;
