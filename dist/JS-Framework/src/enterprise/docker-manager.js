...
`;
    ...
    async startGrid() {
        ...
        return new Promise((resolve, reject) => {
            this.composeProcess = (0, child_process_1.spawn)('docker-compose', ['-f', composePath, 'up', '-d'], {
                cwd: this.outputDir,
    ...
    async stopGrid() {
        ...
        const composePath = path.join(this.outputDir, 'docker-compose.yml');
        return new Promise((resolve, reject) => {
            const stopProcess = (0, child_process_1.spawn)('docker-compose', ['-f', composePath, 'down', '--remove-orphans'], {
                cwd: this.outputDir,
                stdio: ['pipe', 'pipe', 'pipe']
            });
    ...
    async getGridStatus() {
        ...
        try {
            ...
            const { stdout } = await execAsync('docker ps -a --filter "name=QAntum" --format "{{.ID}}|{{.Names}}|{{.Status}}|{{.Ports}}"');
        ...
        try {
            ...
            if (data.value?.nodes) {
                for (const node of data.value.nodes) {
                    ...
                    used += node.slots?.filter((s) => s.session).length || 0;
                }
            }
            return {
                total,
