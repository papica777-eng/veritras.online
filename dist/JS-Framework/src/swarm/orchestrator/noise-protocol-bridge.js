...
exports.NoiseProtocolBridge = void 0;
exports.createNoiseBridge = createNoiseBridge;
 ...
 */
class NoiseProtocolBridge extends events_1.EventEmitter {
    config;
    handshakeState = null;
    sendCipher = null;
    receiveCipher = null;
     ...
     */
    async initiateHandshake(remoteStaticKey) {
     ...
     */
    async processHandshake(message) {
     ...
     */
    getMessagePatterns(pattern, role) {
        const patterns = {
            'NN': ['e', 'e,ee'],
            'NK': ['e,es', 'e,ee'],
            'NX': ['e', 'e,ee,s,es'],
        ...
        for (const token of tokens) {
            switch (token) {
                ...
                case 's':
                    ...
                    const staticKey = this.encryptAndHash(this.handshakeState.localStatic.publicKey);
                    parts.push(staticKey);
                    break;
                case 'ee':
                    this.mixKey(this.dh(this.handshakeState.localEphemeral.privateKey, this.handshakeState.remoteEphemeral));
                    break;
                case 'es':
                    if (this.handshakeState.role === 'initiator') {
                        this.mixKey(this.dh(this.handshakeState.localEphemeral.privateKey, this.handshakeState.remoteStatic));
                    }
                    else {
                        this.mixKey(this.dh(this.handshakeState.localStatic.privateKey, this.handshakeState.remoteEphemeral));
                    }
                    break;
                case 'se':
                    if (this.handshakeState.role === 'initiator') {
                    ...
                    break;
                case 'ss':
                    this.mixKey(this.dh(this.handshakeState.localStatic.privateKey, this.handshakeState.remoteStatic));
                    break;
            }
        }
     ...
     * Process incoming handshake message
     */
    processIncomingHandshakeMessage(message) {
        if (!this.handshakeState) {
    ...
    encrypt(plaintext) {
        ...
        // Create cipher
        const cipher = crypto.createCipheriv('aes-256-gcm', this.sendCipher.key, nonce);
        // Encrypt
        ...
        if (this.encryptionTimings.length > 100)
            this.encryptionTimings.shift();
    ...
    decrypt(message) {
        ...
        const startTime = process.hrtime.bigint();
        // Create decipher
        const decipher = crypto.createDecipheriv('aes-256-gcm', this.receiveCipher.key, message.nonce);
        ...
        if (this.decryptionTimings.length > 100)
            this.decryptionTimings.shift();
    ...
    mixHash(data) {
        if (this.handshakeState) {
            this.handshakeState.handshakeHash = this.hash(Buffer.concat([this.handshakeState.handshakeHash, data]));
        }
    }
    /**
     ...
     */
    rotateKeys() {
        if (!this.sendCipher || !this.receiveCipher)
            return;
...
exports.NoiseProtocolBridge = NoiseProtocolBridge;
// Factory function
function createNoiseBridge(config) {
    return new NoiseProtocolBridge(config);
}
exports.default = NoiseProtocolBridge;
