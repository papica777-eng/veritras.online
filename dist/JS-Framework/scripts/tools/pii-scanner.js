...
exports.PII_PATTERNS = exports.PIIScanner = void 0;
 ...
 */
class PIIScanner extends events_1.EventEmitter {
    config;
    customPatterns = [];
    detections = [];
    constructor(config = {}) {
        super();
        this.config = {
            categories: config.categories ?? Object.keys(PII_PATTERNS),
    ...
    scanText(text, fieldPath = '') {
        ...
        for (const custom of this.customPatterns) {
            ...
            while ((match = custom.pattern.exec(text)) !== null) {
                ...
                if (custom.validator && !custom.validator(value))
                    continue;
                const detection = {
                    id: this.generateId(value, custom.category),
                    category: custom.category,
                    value,
                    redactedValue: this.redact(value, custom.category),
                    confidence: 85,
     ...
     */
    async scanAPIResponse(endpoint, method, responseBody, headers = {}) {
     ...
     */
    checkFieldName(fieldName, parentPath) {
     ...
     */
    calculateConfidence(category, value) {
     ...
     */
    calculateEndpointRiskScore(detections) {
        const riskWeights = {
            critical: 30,
            high: 20,
            medium: 10,
    ...
    analyzeCompliance(detections) {
        const frameworkViolations = new Map();
        for (const detection of detections) {
            for (const framework of detection.compliance) {
                if (!frameworkViolations.has(framework)) {
                    frameworkViolations.set(framework, new Set());
                }
                frameworkViolations.get(framework).add(`${detection.category} exposure at ${detection.fieldPath}`);
            }
        }
    ...
    generateReport(target, startTime) {
        ...
        const criticalDetections = this.detections.filter(d => d.riskLevel === 'critical' || d.riskLevel === 'high');
        // Group by endpoint
        ...
        const criticalEndpoints = Array.from(endpointMap.entries()).map(([endpoint, dets]) => ({
            endpoint,
            method: 'UNKNOWN',
            detections: dets,
