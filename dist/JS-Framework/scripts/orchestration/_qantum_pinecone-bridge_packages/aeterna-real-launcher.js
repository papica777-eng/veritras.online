...
// ════════════════════════════════════════════════════════════
async function compileAeterna(stats) {
...
// ════════════════════════════════════════════════════════════
async function startWebServer(stats, compileInfo) {
    const PORT = 9999;
    const html = `
<!DOCTYPE html>
<html lang="en">
...
async function cloneOrPullRepo() {
    ...
    if (fs.existsSync(WORK_DIR)) {
        ...
        try {
            (0, child_process_1.execSync)(`git clone ${AETERNA_REPO} "${WORK_DIR}"`, {
                stdio: 'inherit'
            });
...
async function checkRustInstalled() {
    ...
    try {
        const version = (0, child_process_1.execSync)('cargo --version', { encoding: 'utf-8' });
...
async function checkNodeInstalled() {
    console.log('\n🔍 Проверка за Node.js...');
    try {
        const version = (0, child_process_1.execSync)('node --version', { encoding: 'utf-8' });
...
function startRustServer() {
    ...
    const proc = (0, child_process_1.spawn)(execPath, [], {
