...
exports.NeuralFingerprintingEngine = void 0;
exports.createNeuralFingerprinting = createNeuralFingerprinting;
...
// NEURAL FINGERPRINTING ENGINE
// ============================================================
class NeuralFingerprintingEngine extends events_1.EventEmitter {
    config;
...
╚═══════════════════════════════════════════════════════════════╝
`);
        Logger_1.default.debug(`   Base personality: ${this.config.basePersonality}`);
        Logger_1.default.debug(`   Device category: ${this.config.deviceCategory}`);
        Logger_1.default.debug(`   Region: ${this.config.region}`);
        Logger_1.default.debug(`   Rotation: ${this.config.rotationStrategy}`);
    }
    /**
    ...
    async generateFingerprint(sessionId) {
        ...
        Logger_1.default.debug(`   ✅ Fingerprint generated: ${fingerprint.fingerprintId}`);
        Logger_1.default.debug(`   📱 Device: ${fingerprint.device.screenResolution.width}x${fingerprint.device.screenResolution.height}`);
        Logger_1.default.debug(`   🌐 Browser: ${fingerprint.browser.userAgent.split(' ')[0]}`);
        Logger_1.default.debug(`   ⌨️ Typing: ${fingerprint.behavior.typingSpeed.wpm} WPM`);
    ...
    generateBehavioralProfile() {
        ...
        return {
            ...
            scrollPattern: this.pickRandom(['smooth', 'stepped', 'variable']),
            typingSpeed: base.typingSpeed,
            keyHoldDuration: { min: 50, max: 150 },
            keyInterval: { min: 80, max: 200 },
            errorRate: base.errorRate,
            correctionPattern: this.pickRandom(['backspace', 'select-delete', 'mixed']),
    ...
    generateNetworkProfile() {
        return {
            connectionType: this.pickRandom(['wifi', 'ethernet', '4g']),
            effectiveType: this.pickRandom(['3g', '4g']),
    ...
    generateMousePath(start, end, options = {}) {
        ...
        for (let i = 0; i <= steps; i++) {
            ...
            if (Math.random() < behavior.hesitationProbability * 0.1) {
                timestamp += this.randomInRange(behavior.microPauseDuration.min, behavior.microPauseDuration.max);
            }
        }
     ...
     */
    generateTypingSequence(text, options = {}) {
        const sequence = [];
        const behavior = this.activeFingerprint?.behavior || this.generateBehavioralProfile();
     ...
     */
    generateScrollSequence(totalDistance, direction) {
        const sequence = [];
        const behavior = this.activeFingerprint?.behavior || this.generateBehavioralProfile();
        ...
        while (remaining > 0) {
            ...
            remaining -= delta;
            // Variable timing
            const interval = this.randomInRange(behavior.scrollSpeed.min, behavior.scrollSpeed.max);
            timestamp += interval;
            sequence.push({
                delta: delta * sign,
                timestamp
    ...
    async rotateFingerprint() {
        Logger_1.default.debug('🔄 Rotating fingerprint...');
...
// EXPORTS
// ============================================================
function createNeuralFingerprinting(config) {
