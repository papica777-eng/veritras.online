...
exports.Hybrid_qantum_cli = void 0;
const HybridGodModeWrapper_1 = require("./HybridGodModeWrapper");
/**
 * @wrapper Hybrid_qantum_cli
 * @description Auto-generated God-Mode Hybrid.
 * @origin "qantum-cli.js"
 */
class Hybrid_qantum_cli extends HybridGodModeWrapper_1.HybridGodModeWrapper {
    async execute() {
        try {
            console.log("/// [HYBRID_CORE] Executing Logics from Hybrid_qantum_cli ///");
...
`);
                ...
                async askCommand(args) {
                    ...
                    // Direct Gemini implementation to bypass missing toolchain
                    const apiKey = process.env.DEEPSEEK_API_KEY || process.env.GEMINI_API_KEY;
                    const model = process.env.DEEPSEEK_MODEL || 'gemini-1.5-flash';
                    if (!apiKey) {
                        console.log(c.error('   ❌ No API Key found in .env'));
                        console.log(c.dim('   Please set GEMINI_API_KEY or DEEPSEEK_API_KEY'));
                        return;
                    }
                    const contents = [{
                            role: 'user',
                            parts: [{ text: "You are QAntum Prime Sovereign AI. " + args }]
                        }];
...
`);
            ...
            await this.recordAxiom({
                status: 'SUCCESS',
                origin: 'Hybrid_qantum_cli',
                timestamp: Date.now()
            });
        }
        catch (error) {
            console.error("/// [HYBRID_FAULT] Critical Error in Hybrid_qantum_cli ///", error);
            await this.recordAxiom({
...
exports.Hybrid_qantum_cli = Hybrid_qantum_cli;
