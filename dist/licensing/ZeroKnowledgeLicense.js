...
class ZeroKnowledgeLicense extends events_1.EventEmitter {
    ...
    createLicense(tier, expirationDate) {
        ...
        const tierCommitment = this.pedersenCommit(BigInt(tierValue), this.hashToScalar(blindingFactors.tier));
        // Create commitment for expiration
        const expirationTimestamp = Math.floor(expirationDate.getTime() / 1000);
        const expirationCommitment = this.pedersenCommit(BigInt(expirationTimestamp), this.hashToScalar(blindingFactors.expiration));
        // Get tier limits
