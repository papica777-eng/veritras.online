...
exports.getToolSelector = exports.ToolSelector = void 0;
 ...
 * ```
 */
class ToolSelector extends events_1.EventEmitter {
    static instance;
    config;
    registry;
    // Selection cache
     ...
     */
    async selectMultipleTools(query, count = 3) {
        const results = [];
     ...
     */
    quickSelect(category, capability) {
     ...
     */
    getCandidateTools(query, categories) {
        ...
        if (query.requiredCapabilities && query.requiredCapabilities.length > 0) {
            candidates = candidates.filter(t => query.requiredCapabilities.every(cap => t.capabilities.some(c => c.toLowerCase().includes(cap.toLowerCase()))));
        }
     ...
     */
    async scoreCandidates(candidates, query) {
     ...
     */
    calculateSemanticScore(tool, task) {
     ...
     */
    calculateCapabilityScore(tool, query) {
        if (!query.requiredCapabilities || query.requiredCapabilities.length === 0) {
            return 0.5; // Neutral score
        }
        let matches = 0;
        for (const required of query.requiredCapabilities) {
            if (tool.capabilities.some(c => c.toLowerCase().includes(required.toLowerCase()))) {
                matches++;
            }
        }
     ...
     */
    recordOutcome(query, toolId, success) {
     ...
     */
    getToolSelectionSuccessRate(toolId) {
