...
${RESET}`);
    ...
    console.log('');
    // ==========================================================
    // PHASE 1.5: GHOST PERSONALITY INJECTION (Anti-Bot Evasion)
    // ==========================================================
    logger_1.logger.info(`${BOLD}PHASE 1.5: Booting Ghost Personality Engine...${RESET}`);
    await sleep(500);
    const ghostEngine = new personality_engine_1.GhostPersonalityEngine();
    // Access the private method via any for demo purposes or use public if available
    const ghost = ghostEngine.createFromArchetype('careful');
    console.log(`${MAGENTA}[GHOST-ENGINE]${RESET} Ingesting Human DNA Traits...`);
    await sleep(800);
    console.log(`${MAGENTA}[GHOST-ENGINE]${RESET} Generated Persona: ${BOLD}"${ghost.name}"${RESET}`);
    console.log(`${MAGENTA}[GHOST-ENGINE]${RESET} Behavioral Vectors:`);
    console.log(`  - Mouse Speed: ${ghost.mouse.averageSpeed} px/s (Curve Intensity: ${ghost.mouse.curveIntensity.toFixed(2)})`);
    console.log(`  - Typing Rhythm: ${ghost.typing.wordsPerMinute} WPM (Error Rate: ${(ghost.typing.errorRate * 100).toFixed(1)}%)`);
    console.log(`  - Hesitation Freq: ${(ghost.mouse.hesitationFrequency * 100).toFixed(1)}%`);
    console.log(`${GREEN}✅ Evasion DNA successfully mapped to Swarm Workers.${RESET}`);
    console.log('');
    // ==========================================================
