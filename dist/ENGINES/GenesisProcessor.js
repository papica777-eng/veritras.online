...
const GenesisRealityProvider_js_1 = require("./GenesisRealityProvider.js");
...
async function processGenesisTestJob(job) {
    ...
    if (limits.maxRealities !== -1) {
        const activeRealities = GenesisRealityProvider_js_1.genesisRealityProvider.getActiveRealities(tenantId);
    ...
    try {
        ...
        manifestation = await GenesisRealityProvider_js_1.genesisRealityProvider.manifestReality(realitySpec);
    ...
    for (let i = 0; i < tests.length; i += parallelism) {
        ...
        const reality = GenesisRealityProvider_js_1.genesisRealityProvider.getReality(manifestation.id);
    ...
    if (config.collapseAfterRun) {
        try {
            ...
            await GenesisRealityProvider_js_1.genesisRealityProvider.collapseReality(manifestation.id);
...
async function executeGenesisTest(manifestation, test, config, realitySpec) {
    ...
    try {
        ...
        // Execute observation
        const observation = await GenesisRealityProvider_js_1.genesisRealityProvider.observeReality({
...
async function executeHybridGenesisTest(manifestation, test, config) {
    ...
    try {
        ...
        ${test.code}
      `;
            const result = await executor_js_1.ghostExecutor.executeTest(context.sessionId, enhancedTestCode, test.timeout);
            return {
                testId: test.id,
            ...
            await executor_js_1.ghostExecutor.cleanup(context.sessionId);
        }
    }
    catch (error) {
        return {
