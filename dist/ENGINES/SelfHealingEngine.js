...
// SELF-HEALING ENGINE
// ═══════════════════════════════════════════════════════════════════════════════
class SelfHealingEngine extends events_1.EventEmitter {
    page;
    config;
    strategies = new Map();
    healingHistory = [];
    ...
    async heal(selector, context) {
        ...
        for (const { selector: altSelector, strategy } of alternatives) {
            ...
            try {
                ...
                if (count > 0) {
                    ...
                    this.emit('healed', result);
                    logger_1.logger.debug(`🔧 Healed: ${selector} → ${altSelector} (${strategy})`);
                    return result;
                }
            }
            catch {
        ...
        logger_1.logger.debug(`❌ Healing failed for: ${selector} (${result.attempts} attempts)`);
        return result;
    }
    /**
     * Find element with auto-healing
     */
    async findWithHealing(selector, options) {
        if (!this.page) {
     ...
     */
    generateAllAlternatives(selector, context) {
        const alternatives = [];
        const seen = new Set();
