...
exports.getVeritasLicenseEngine = exports.VeritasLicenseEngine = exports.VERITAS_LICENSE_TIERS = void 0;
exports.createStripeCheckout = createStripeCheckout;
const crypto = __importStar(require("crypto"));
// ═══════════════════════════════════════════════════════════════════════════════
// LICENSE TIERS
// ═══════════════════════════════════════════════════════════════════════════════
exports.VERITAS_LICENSE_TIERS = {
    'VERITAS-SDK-FREE': {
...
class VeritasLicenseEngine {
     ...
     */
    generateLicense(type, email, durationDays = 365) {
    ...
    validateLicense(key) {
        ...
        if (proMatch) {
            return {
                valid: true,
                type: 'VERITAS-SDK-PRO',
                tier: exports.VERITAS_LICENSE_TIERS['VERITAS-SDK-PRO'],
        ...
        if (freeMatch) {
            return {
                valid: true,
                type: 'VERITAS-SDK-FREE',
                tier: exports.VERITAS_LICENSE_TIERS['VERITAS-SDK-FREE'],
                expiresAt: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000)
            };
        }
        if (entMatch) {
            return {
                valid: true,
                type: 'VERITAS-SDK-ENTERPRISE',
                tier: exports.VERITAS_LICENSE_TIERS['VERITAS-SDK-ENTERPRISE'],
                expiresAt: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000)
            };
        }
        if (trialMatch) {
            return {
                valid: true,
                type: 'TRIAL',
                tier: exports.VERITAS_LICENSE_TIERS['TRIAL'],
     ...
     */
    hasFeature(type, feature) {
        const tier = exports.VERITAS_LICENSE_TIERS[type];
        if (!tier)
            return false;
        const featureMap = {
        ...
        return [
            {
                tier: 'Free',
                price: '$0/mo',
                features: exports.VERITAS_LICENSE_TIERS['VERITAS-SDK-FREE'].features,
                cta: 'Get Started'
            },
            {
                tier: 'Pro',
                price: '$29/mo',
                features: exports.VERITAS_LICENSE_TIERS['VERITAS-SDK-PRO'].features,
                cta: 'Start Pro Trial'
            },
            {
                tier: 'Enterprise',
                price: '$199/mo',
                features: exports.VERITAS_LICENSE_TIERS['VERITAS-SDK-ENTERPRISE'].features,
                cta: 'Contact Sales'
            }
        ];
    }
}
exports.VeritasLicenseEngine = VeritasLicenseEngine;
async function createStripeCheckout(options) {
    // This would integrate with actual Stripe API
    // For now, return a placeholder URL
    const tier = exports.VERITAS_LICENSE_TIERS[options.tier];
...
// EXPORTS
// ═══════════════════════════════════════════════════════════════════════════════
const getVeritasLicenseEngine = () => VeritasLicenseEngine.getInstance();
exports.getVeritasLicenseEngine = getVeritasLicenseEngine;
exports.default = VeritasLicenseEngine;
