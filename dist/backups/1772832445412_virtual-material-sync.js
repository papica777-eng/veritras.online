...
exports.VirtualMaterialSyncEngine = void 0;
exports.createVirtualMaterialSync = createVirtualMaterialSync;
...
// ============================================================
class VirtualMaterialSyncEngine extends events_1.EventEmitter {
    config;
    templates = new Map();
    syncHistory = [];
    singularityCore = null;
    // Template patterns for each provider
    static TEMPLATE_GENERATORS = {
...
`);
        ...
        logger_1.logger.debug(`   ✅ Singularity Core v${this.singularityCore?.version}`);
        logger_1.logger.debug(`   📁 Output directory: ${this.config.outputDir}`);
        logger_1.logger.debug(`   ☁️ Providers: ${this.config.providers.join(', ')}`);
    }
    /**
     * 📦 Load Singularity Core configuration
     */
    // Complexity: O(1)
    async loadSingularityCore() {
     ...
     */
    // Complexity: O(N) — loop
    async syncProviderTemplates(provider, vars) {
        const results = [];
        ...
        for (const templateType of templateTypes) {
            try {
                const result = await this.generateAndSaveTemplate(provider, templateType, vars);
                results.push(result);
            }
            catch (error) {
                results.push({
     ...
     */
    // Complexity: O(1)
    getTemplateTypesForProvider(provider) {
        switch (provider) {
            case 'docker':
     ...
     */
    // Complexity: O(N)
    async generateAndSaveTemplate(provider, templateType, vars) {
        ...
        logger_1.logger.debug(`   ✅ ${provider}/${templateType}: updated`);
        return {
            templateId,
            provider,
     ...
     */
    // Complexity: O(1)
    getGenerator(provider, templateType) {
     ...
     */
    // Complexity: O(1)
    getTemplateFilePath(provider, templateType) {
        const extensions = {
     ...
     */
    // Complexity: O(1)
    async validateTemplate(provider, templateType, content) {
        const errors = [];
        // Basic validation
    ...
    detectChanges(oldContent, newContent) {
        ...
        if (added > 0)
            changes.push(`+${added} lines added`);
        if (removed > 0)
            changes.push(`-${removed} lines removed`);
        return changes;
    }
    /**
     ...
     * 🐳 Generate Dockerfile
     */
    static generateDockerTemplate(core, vars) {
...
`;
     ...
     */
    static generateDockerComposeTemplate(core, vars) {
        return `# QANTUM QA Framework v${core.version}
# Auto-generated by Virtual Material Sync Engine
...
`;
     ...
     */
    static generateAWSTemplate(core, vars) {
...
`;
     ...
     */
    static generateAWSLambdaTemplate(core, vars) {
        return `AWSTemplateFormatVersion: '2010-09-09'
...
`;
    ...
    static generateECSTemplate(core, vars) {
    ...
    static generateECRTemplate(core, vars) {
        return JSON.stringify({
            Version: '2012-10-17',
            Statement: [{
                    Effect: 'Allow',
     ...
     */
    static generateAzureTemplate(core, vars) {
        return JSON.stringify({
    ...
    static generateAzureFunctionsTemplate(core, vars) {
        return JSON.stringify({
            bindings: [{
                    type: "httpTrigger",
                    direction: "in",
    ...
    static generateAzureContainerAppsTemplate(core, vars) {
...
`;
     ...
     */
    static generateGCPTemplate(core, vars) {
...
`;
    ...
    static generateGCPFunctionsTemplate(core, vars) {
...
`;
    ...
    static generateGKETemplate(core, vars) {
...
`;
     ...
     */
    static generateKubernetesTemplate(core, vars) {
        return `apiVersion: apps/v1
kind: Deployment
metadata:
  name: QANTUM-mind
...
`;
    ...
    static generateK8sServiceTemplate(core, vars) {
...
`;
    ...
    static generateK8sConfigMapTemplate(core, vars) {
        return `apiVersion: v1
kind: ConfigMap
metadata:
...
`;
    ...
    static generateK8sIngressTemplate(core, vars) {
...
`;
    ...
    static generateK8sHPATemplate(core, vars) {
...
exports.VirtualMaterialSyncEngine = VirtualMaterialSyncEngine;
// ============================================================
// EXPORTS
// ============================================================
function createVirtualMaterialSync(config) {
