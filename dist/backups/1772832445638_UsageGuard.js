...
exports.UsageGuard = exports.RATE_LIMIT_PLANS = void 0;
exports.getUsageGuard = getUsageGuard;
exports.createUsageGuard = createUsageGuard;
const events_1 = require("events");
// ═══════════════════════════════════════════════════════════════════════════════════════════
// PLAN CONFIGURATIONS
// ═══════════════════════════════════════════════════════════════════════════════════════════
exports.RATE_LIMIT_PLANS = {
    starter: {
        tier: 'starter',
        requestsPerSecond: 2,
 ...
 * ```
 */
class UsageGuard extends events_1.EventEmitter {
    config;
    buckets = new Map();
    ...
    checkLimit(clientId, tier) {
        const plan = exports.RATE_LIMIT_PLANS[tier];
        const now = Date.now();
     ...
     */
    // Complexity: O(1) — lookup
    getOrCreateBucket(clientId, plan) {
    ...
    // ─────────────────────────────────────────────────────────────────────────────────────────
    // Complexity: O(1)
    createAllowedResult(plan, usedBurst = false) {
        return {
            allowed: true,
    ...
    // Complexity: O(1)
    createThrottledResult(reason, plan, bucket, retryAfter) {
    ...
    // Complexity: O(1)
    createBlockedResult(reason, plan, retryAfter) {
     ...
     */
    // Complexity: O(N)
    setCustomLimits(clientId, limits) {
 ...
 */
function createUsageGuard(config) {
    return new UsageGuard(config);
}
exports.default = UsageGuard;
