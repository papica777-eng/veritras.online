...
exports.BiometricEngine = void 0;
exports.getBiometricEngine = getBiometricEngine;
exports.createBiometricEngine = createBiometricEngine;
// ═══════════════════════════════════════════════════════════════════════════════
...
class BezierEngine {
     ...
     */
    static generateControlPoints(start, end, intensity) {
...
class BiometricEngine {
     ...
     */
    // Complexity: O(1)
    calculateMovementTime(distance, targetWidth = 20) {
    ...
    // Complexity: O(1)
    calculateTimestamp(t, totalDuration) {
        // Ease-in-out timing
        const eased = t < 0.5
            ? 4 * t * t * t
    ...
    // Complexity: O(1)
    calculateInterKeyDelay(prevChar, currentChar, baseDelay) {
        let modifier = 1;
        // Same finger = slower
     ...
     */
    // Complexity: O(N*M) — nested iteration
    generateScrollPattern(totalDistance, direction = 'down') {
        const pattern = [];
        ...
        if (behavior === 'smooth') {
            ...
            while (remaining > 0) {
                const scrollAmount = Math.min(remaining, this.gaussian.bounded(100, 40, 30, 200));
                pattern.push({
            ...
            while (remaining > 0) {
                const numSteps = Math.min(Math.ceil(remaining / wheelDelta), Math.floor(this.gaussian.bounded(3, 1, 1, 6)));
     ...
     */
    // Complexity: O(1)
    generateReadingDelay(textLength, wordsPerMinute = 250) {
     ...
     */
    // Complexity: O(N) — loop
    async executeMouseMove(page, from, to) {
     ...
     */
    // Complexity: O(1)
    async executeClick(page, point, options) {
     ...
     */
    // Complexity: O(N) — loop
    async executeType(page, selector, text) {
     ...
     */
    // Complexity: O(N) — loop
    async executeScroll(page, distance, direction = 'down') {
...
exports.BiometricEngine = BiometricEngine;
// ═══════════════════════════════════════════════════════════════════════════════
// SINGLETON & FACTORY
// ═══════════════════════════════════════════════════════════════════════════════
let defaultEngine = null;
function getBiometricEngine(config) {
    if (!defaultEngine) {
        defaultEngine = new BiometricEngine(config);
    }
    return defaultEngine;
}
function createBiometricEngine(config) {
    return new BiometricEngine(config);
}
exports.default = BiometricEngine;
