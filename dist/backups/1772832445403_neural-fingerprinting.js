...
╚═══════════════════════════════════════════════════════════════╝
`);
        logger_1.logger.debug(`   Base personality: ${this.config.basePersonality}`);
        logger_1.logger.debug(`   Device category: ${this.config.deviceCategory}`);
        logger_1.logger.debug(`   Region: ${this.config.region}`);
        logger_1.logger.debug(`   Rotation: ${this.config.rotationStrategy}`);
    }
    /**
    ...
    async generateFingerprint(sessionId) {
        ...
        logger_1.logger.debug(`   ✅ Fingerprint generated: ${fingerprint.fingerprintId}`);
        logger_1.logger.debug(`   📱 Device: ${fingerprint.device.screenResolution.width}x${fingerprint.device.screenResolution.height}`);
        logger_1.logger.debug(`   🌐 Browser: ${fingerprint.browser.userAgent.split(' ')[0]}`);
        logger_1.logger.debug(`   ⌨️ Typing: ${fingerprint.behavior.typingSpeed.wpm} WPM`);
    ...
    async rotateFingerprint() {
        logger_1.logger.debug('🔄 Rotating fingerprint...');
