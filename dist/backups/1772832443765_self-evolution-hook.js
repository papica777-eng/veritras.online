...
exports.SelfEvolutionHookEngine = void 0;
exports.createSelfEvolutionHook = createSelfEvolutionHook;
...
// SELF-EVOLUTION HOOK ENGINE
// ============================================================
class SelfEvolutionHookEngine extends events_1.EventEmitter {
    config;
    ...
    static TRANSFORMATIONS = {
        ...
        addParameter: (code, funcName, newParam, defaultValue) => {
        ...
        addDeprecationWarning: (code, funcName, replacement) => {
...
`);
        ...
        if (this.config.gitEnabled) {
            try {
                ...
                logger_1.logger.debug('   ✅ Git integration enabled');
            }
            catch {
                logger_1.logger.debug('   ⚠️ Git not available, disabling auto-commit');
                this.config.gitEnabled = false;
            }
        }
        logger_1.logger.debug(`   ✅ Auto-commit: ${this.config.autoCommit ? 'ENABLED' : 'DISABLED'}`);
        logger_1.logger.debug(`   ✅ Max commits/hour: ${this.config.maxAutoCommitsPerHour}`);
        logger_1.logger.debug(`   ✅ Backup directory: ${this.config.backupDirectory}`);
    }
    /**
    ...
    registerTrigger(trigger) {
        ...
        logger_1.logger.debug(`   Severity: ${fullTrigger.severity}`);
        logger_1.logger.debug(`   Source: ${fullTrigger.source}`);
     ...
     */
    // Complexity: O(N)
    async createEvolutionPlan(trigger) {
        ...
        logger_1.logger.debug(`   ✅ Plan created: ${plan.planId}`);
        logger_1.logger.debug(`   📁 Affected files: ${affectedFiles.length}`);
        logger_1.logger.debug(`   🔧 Changes: ${changes.length}`);
        logger_1.logger.debug(`   ⚠️ Requires review: ${requiresReview}`);
     ...
     */
    // Complexity: O(N)
    async analyzeAffectedFiles(trigger) {
        const files = [];
    ...
    async generateChanges(trigger, affectedFiles) {
        const changes = [];
            ...
            switch (trigger.type) {
                ...
                case 'api_update':
                    if (trigger.context.newParameter) {
                        ...
                        newCode = SelfEvolutionHookEngine.TRANSFORMATIONS.addParameter(originalCode, trigger.context.functionName || '', trigger.context.newParameter, trigger.context.defaultValue || 'undefined');
                    }
                    break;
                case 'deprecation':
                    description = `Add deprecation warning for: ${trigger.context.functionName}`;
                    newCode = SelfEvolutionHookEngine.TRANSFORMATIONS.addDeprecationWarning(originalCode, trigger.context.functionName || '', trigger.context.replacement || 'newFunction');
                    break;
                case 'security_fix':
                    description = `Add error handling to: ${trigger.context.functionName}`;
                    newCode = SelfEvolutionHookEngine.TRANSFORMATIONS.addErrorHandling(originalCode, trigger.context.functionName || '');
                    breakingChange = true;
                    break;
                case 'optimization':
     ...
     */
    // Complexity: O(1)
    addFieldToInterface(code, context) {
     ...
     */
    // Complexity: O(1)
    determineStrategy(trigger, changes) {
    ...
    approvePlan(planId) {
        ...
        logger_1.logger.debug(`✅ Plan ${planId} approved`);
    ...
    async executePlan(planId) {
        ...
        try {
            ...
            if (plan.strategy.createBackup) {
                ...
                logger_1.logger.debug(`   📦 Backup created: ${result.backupPath}`);
            }
            // Apply changes
            for (const change of plan.changes) {
                try {
                    ...
                    logger_1.logger.debug(`   ✅ Modified: ${change.filePath}`);
                }
                catch (error) {
            ...
            if (this.config.gitEnabled && this.config.autoCommit) {
                ...
                logger_1.logger.debug(`   📝 Committed: ${result.commitHash}`);
            }
            result.success = true;
            plan.status = 'committed';
            this.executedPlans.push(plan);
            this.pendingPlans.delete(planId);
            logger_1.logger.debug(`   ✅ Evolution complete!`);
    ...
    async createBackup(plan) {
        ...
        // Save plan metadata
        fs.writeFileSync(path.join(backupPath, 'plan.json'), JSON.stringify(plan, null, 2));
        return backupPath;
    }
    /**
     ...
     */
    // Complexity: O(N) — loop
    async rollback(plan, backupPath) {
        logger_1.logger.debug(`   ⏪ Rolling back changes...`);
        for (const change of plan.changes) {
            const backupFile = path.join(backupPath, change.filePath.replace(/\//g, '_'));
        ...
        logger_1.logger.debug(`   ✅ Rollback complete`);
    }
    /**
     * 📝 Commit changes to Git
     */
    // Complexity: O(N) — linear scan
    async commitChanges(plan) {
        // Check rate limit
        ...
        try {
            ...
            // Commit
            const { stdout } = await execAsync(`git commit -m "${message}"`, { cwd: this.config.workingDirectory });
            // Get commit hash
            const { stdout: hashOut } = await execAsync('git rev-parse HEAD', { cwd: this.config.workingDirectory });
     ...
     */
    // Complexity: O(1)
    async quickEvolve(type, source, context, options = {}) {
        // Register trigger
        ...
        if (options.skipApproval || !plan.requiresReview) {
            ...
            logger_1.logger.debug(`   ⏸️ Plan requires manual approval`);
            return null;
        }
        // Execute
    ...
    calculateSuccessRate() {
        if (this.executedPlans.length === 0)
            return 1;
...
exports.SelfEvolutionHookEngine = SelfEvolutionHookEngine;
// ============================================================
// EXPORTS
// ============================================================
function createSelfEvolutionHook(config) {
