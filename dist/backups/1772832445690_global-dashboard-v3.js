...
exports.GlobalDashboardV3 = void 0;
exports.createGlobalDashboard = createGlobalDashboard;
...
`);
        ...
        logger_1.logger.debug(`🌍 [DASHBOARD] HTTP Server: http://localhost:${this.config.port}`);
        logger_1.logger.debug(`🌍 [DASHBOARD] WebSocket: ws://localhost:${this.config.wsPort}`);
        logger_1.logger.debug(`🌍 [DASHBOARD] Theme: ${this.config.theme.toUpperCase()}`);
        logger_1.logger.debug('');
    }
    /**
    ...
    async startWebSocketServer() {
        this.wsServer = new ws_1.WebSocketServer({ port: this.config.wsPort });
        this.wsServer.on('connection', (ws) => {
            this.clients.add(ws);
            logger_1.logger.debug(`🌍 [DASHBOARD] Client connected (${this.clients.size} total)`);
            // Send initial state
            ...
            ws.on('close', () => {
                ...
                logger_1.logger.debug(`🌍 [DASHBOARD] Client disconnected (${this.clients.size} total)`);
            });
        });
    }
    /**
     ...
     */
    // Complexity: O(1) — lookup
    addTestFlow(fromNodeId, toNodeId, testName) {
        const flow = {
            id: `flow_${Date.now()}`,
            from: fromNodeId,
            to: toNodeId,
...
<body>
    ...
    generateWorldMapSVG() {
        return `<svg viewBox="0 0 1000 500" xmlns="http://www.w3.org/2000/svg">
            ...
            <g stroke="#00d4ff" stroke-width="0.5" fill="none" opacity="0.3">
                <!-- Grid lines -->
                ${Array.from({ length: 19 }, (_, i) => `<line x1="0" y1="${i * 27.78}" x2="1000" y2="${i * 27.78}"/>`).join('')}
                ${Array.from({ length: 37 }, (_, i) => `<line x1="${i * 27.78}" y1="0" x2="${i * 27.78}" y2="500"/>`).join('')}
            </g>
    ...
    stop() {
        ...
        logger_1.logger.debug('🌍 [DASHBOARD] Stopped');
    }
}
...
// EXPORTS
// ============================================================
function createGlobalDashboard(config) {
