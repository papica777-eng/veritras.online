...
exports.TLSPhantomEngine = void 0;
exports.getTLSPhantom = getTLSPhantom;
exports.createTLSPhantom = createTLSPhantom;
const events_1 = require("events");
const crypto = __importStar(require("crypto"));
// ═══════════════════════════════════════════════════════════════════════════════
// REAL BROWSER TLS PROFILES - Chrome, Firefox, Safari, Edge
// ═══════════════════════════════════════════════════════════════════════════════
const CHROME_121_PROFILE = {
    id: 'chrome-121',
...
// TLS PHANTOM ENGINE
// ═══════════════════════════════════════════════════════════════════════════════
class TLSPhantomEngine extends events_1.EventEmitter {
    config;
    currentProfile;
    profileIndex = 0;
    rotationTimer = null;
    ...
    // Complexity: O(N) — linear scan
    signatureAlgorithmsToOpenSSL(sigalgs) {
        const mapping = {
            0x0401: 'RSA+SHA256',
            0x0403: 'ECDSA+SHA256',
            0x0501: 'RSA+SHA384',
    ...
    // Complexity: O(N) — linear scan
    supportedGroupsToOpenSSL(groups) {
        const mapping = {
            29: 'X25519',
            23: 'P-256',
            24: 'P-384',
...
exports.TLSPhantomEngine = TLSPhantomEngine;
// ═══════════════════════════════════════════════════════════════════════════════
// SINGLETON & FACTORY
// ═══════════════════════════════════════════════════════════════════════════════
let defaultEngine = null;
function getTLSPhantom(config) {
    if (!defaultEngine) {
        defaultEngine = new TLSPhantomEngine(config);
    }
    return defaultEngine;
}
function createTLSPhantom(config) {
    return new TLSPhantomEngine(config);
}
exports.default = TLSPhantomEngine;
