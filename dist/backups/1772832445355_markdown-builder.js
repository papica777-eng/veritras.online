...
class MarkdownBuilder {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // Complexity: O(1)
    buildHeader(projectName, version) {
        return `
# ⚛️ ${projectName}
...
`;
    ...
    // Complexity: O(N) — linear scan
    buildFunctionReference(func) {
        const params = func.parameters.map(p => `- \`${p.name}\` (\`${p.type}\`${p.optional ? ', optional' : ''}) - ${p.defaultValue ? `Default: ${p.defaultValue}` : 'Required parameter'}`).join('\n');
        return `
#### \`${func.name}()\`
...
`;
    ...
    // Complexity: O(N) — linear scan
    buildInterfaceReference(iface) {
        const props = iface.properties.map(p => `| \`${p.name}\` | \`${p.type}\` | ${p.optional ? 'No' : 'Yes'} | ${p.description || '-'} |`).join('\n');
        return `
#### \`${iface.name}\`
