...
`);
        ...
        logger_1.logger.debug(`   📁 Storage: ${this.config.profileStoragePath}`);
        logger_1.logger.debug(`   📊 Loaded profiles: ${this.activatedProfiles.size}`);
        logger_1.logger.debug(`   🔄 Auto-activate: ${this.config.autoActivateNewAccounts}`);
    }
    /**
    ...
    async loadPersistedProfiles() {
        ...
        for (const file of files) {
            try {
                ...
                logger_1.logger.warn(`   ⚠️ Failed to load profile: ${file}`);
            }
        }
