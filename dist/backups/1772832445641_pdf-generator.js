...
exports.ExecutivePDFReport = void 0;
...
class ExecutivePDFReport {
    ...
    addCoverPage(data) {
        ...
        this.doc.text(`Generated: ${data.reportDate.toLocaleDateString('en-US', {
            weekday: 'long',
            year: 'numeric',
            month: 'long',
            day: 'numeric'
    ...
    addExecutiveSummary(data) {
        ...
        // Summary text
        this.doc.text(`This report provides a comprehensive overview of your automated testing infrastructure powered by QANTUM AI Engine. ` +
            `During the reporting period, the system executed ${metrics.totalTests.toLocaleString()} tests with a ${metrics.passRate.toFixed(1)}% success rate.`, 50, 120, { width: 495, align: 'justify' });
        // Highlight boxes
        let y = 180;
        ...
        this.doc.text(`Based on test patterns, we recommend focusing on the Payment module which shows 12% higher failure rate. ` +
            `Implementing additional coverage here could prevent potential revenue loss.`, 60, y + 28, { width: 470 });
    }
    /**
    ...
    addROIAnalysis(data) {
        ...
        const { roi } = data;
        this.doc.fontSize(11).fillColor('#ffffff');
        this.doc.text(`This section quantifies the business value delivered by QANTUM automation. ` +
            `All calculations are based on industry-standard metrics and your organization's cost structure.`, 50, 120, { width: 495, align: 'justify' });
        // ROI Summary Box
    ...
    addSelfHealingReport(data) {
        ...
        this.doc.fontSize(11).fillColor('#ffffff');
        this.doc.text(`The QANTUM AI engine automatically healed ${data.metrics.healed} broken selectors during this period, ` +
            `preventing test failures and maintaining suite stability without manual intervention.`, 50, 120, { width: 495, align: 'justify' });
        // Healing strategies
        let y = 180;
        this.doc.fontSize(12).fillColor(this.colors.cyan);
    ...
    // Complexity: O(1)
    addProgressBar(label, value, total, color, y) {
...
exports.ExecutivePDFReport = ExecutivePDFReport;
// ============================================================
// EXPORT FUNCTION
// ============================================================
async function generateExecutiveReport(outputDir = './reports') {
...
if (require.main === module) {
    ...
    generateExecutiveReport('./reports')
        .then(path => logger_1.logger.debug(`✅ Report generated: ${path}`))
        .catch(err => logger_1.logger.error('❌ Failed to generate report:', err));
}
