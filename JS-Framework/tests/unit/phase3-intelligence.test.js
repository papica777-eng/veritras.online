...
describe('Step 44: SEGCController', () => {
  ...
  class GeneticMutationEngine {
    ...
    getPendingMutations() { return [...this.pendingMutations]; }
...
describe('Step 47: CognitiveUXAuditor', () => {
  ...
  class CognitiveUXAuditor {
    ...
    getHistory() { return [...this.analysisHistory]; }
    clearHistory() { this.analysisHistory = []; }

    getAverageScore() {
      if (this.analysisHistory.length === 0) return 0;
      return this.analysisHistory.reduce((sum, r) => sum + r.score, 0) / this.analysisHistory.length;
    }
  ...
  test('CognitiveUXAuditor analyze should throw without API key', async () => {
    const auditor = new CognitiveUXAuditor();
    let threw = false;
    try { await auditor.analyze('screenshot'); } catch { threw = true; }
