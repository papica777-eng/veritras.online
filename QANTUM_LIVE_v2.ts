...
// ── HTTP helpers ──
function httpsReq(method: string, reqPath: string, body: string = '', headers: Record<string, string> = {}): Promise<any> {
...
// ── Cognitive Stack ──
import { createQAntumOrchestrator } from './src/ai/AIIntegration';
...
function log(msg: string): void {
    ...
    console.log(`[${ts}] ${msg}`);
    try { fs.appendFileSync('./qantum-live-trades.log', `[${ts}] ${msg}\n`); } catch { }
}
    ...
    while (isRunning) {
        ...
        if (!bAcc.code) {
            for (const b of bAcc.balances) {
                if (b.asset === tradingQuote) currentBalance = parseFloat(b.free);
            }
        }
        ...
        for (const sym of tradingSymbols) {
            ...
            if (hist.length < 3) continue; // Need minimum data
            ...
            if (['EXECUTE', 'MAXIMIZE', 'AGGRESSIVE_ENTRY'].includes(action)) side = 'BUY';
            if (['EVACUATE', 'FULL_DEFENSE', 'DEFENSIVE_POSITION', 'EXPLOIT_PARADOX'].includes(action)) side = 'SELL';
