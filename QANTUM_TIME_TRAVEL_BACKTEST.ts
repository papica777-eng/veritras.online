...
import {
    ...
    FailureType,
    PatchType,
} from './src/engines/ParadoxEngine';
import type {
    ButterflyEffect,
    TimeTravelPatch,
    SimulationState,
...
// ═══════════════════════════════════════════════════════════════════════════════
// HISTORICAL MARKET EPOCHS — REAL PATTERNS
// ═══════════════════════════════════════════════════════════════════════════════
...
interface HistoricalEpoch {
    ...
    description: string;
    prices: number[];
    actualOutcome: 'CRASH' | 'PUMP' | 'FLAT' | 'RECOVERY';
...
// Complexity: O(1)
function actionToDirection(action: string): 'LONG' | 'SHORT' | 'NONE' {
    switch (action) {
        case 'EXECUTE':
        case 'MAXIMIZE':
...
function runCognitiveBacktest(brain: QAntumOrchestrator): {
    results: TradeResult[];
    totalReturn: number;
    winRate: number;
    finalCapital: number;
    ...
    for (let i = 0; i < 4; i++) {
        ...
        brain.recordHistoricalEvent('MARKET', epoch.asset, epoch.prices,
            `${epoch.name} [${epoch.dateRange}]`, epoch.actualOutcome);
    }
    ...
    for (let i = 0; i < HISTORICAL_EPOCHS.length; i++) {
        ...
        const decision = brain.process('MARKET' as any, epoch.asset, epoch.prices);
...
async function runParadoxSimulation(brain: QAntumOrchestrator): Promise<{
    ...
    const paradox = createParadoxEngine({
        timeMultiplier: 10,
        loadMultiplier: 50,
    ...
    let patchCount = 0;
    const butterflyLog: string[] = [];

    // Wire Paradox events to Cognitive Stack
    paradox.on('butterfly:detected', (effect: ButterflyEffect) => {
        butterflyCount++;

        // Feed the butterfly effect through the Cognitive Stack as a SYSTEM signal
        const severityToNumbers: Record<string, number[]> = {
    ...
    paradox.on('patch:applied', (patch: TimeTravelPatch) => {
        patchCount++;
...
function printFinalReport(
    cognitiveResults: ReturnType<typeof runCognitiveBacktest>,
    paradoxResults: Awaited<ReturnType<typeof runParadoxSimulation>>,
    systemReport: any,
    totalTimeMs: number
): void {
    const hr = '='.repeat(76);
