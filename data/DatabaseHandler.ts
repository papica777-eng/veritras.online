...
import { logger } from '../api/unified/utils/logger.js';
// ═══════════════════════════════════════════════════════════════════════════════
// TYPE DEFINITIONS
// ═══════════════════════════════════════════════════════════════════════════════
...
class DatabaseHandler extends EventEmitter {
  ...
  async createLicense(email: string, planId: string): Promise<License> {
    if (!this.knex) throw new Error('Database not connected');
  ...
  async validateLicense(key: string, fingerprint?: string): Promise<{ valid: boolean, reason?: string }> {
    const license = await this.getLicenseByKey(key);
    if (!license) return { valid: false, reason: 'Invalid Key' };
    if (license.status !== 'active') return { valid: false, reason: 'License Suspended' };
    if (new Date() > new Date(license.expires_at)) return { valid: false, reason: 'License Expired' };
  ...
  async initializeSchema(): Promise<void> {
    ...
    if (!await this.knex.schema.hasTable('licenses')) {
      ...
      logger.debug('✅ Created licenses table');
    }
 ...
 */
function createDatabaseHandler(config?: Partial<DatabaseConfig>): DatabaseHandler {
 ...
 */
function createSQLiteHandler(filename: string = './mind-engine.db'): DatabaseHandler {
...
export { DatabaseHandler, createDatabaseHandler, createSQLiteHandler };
